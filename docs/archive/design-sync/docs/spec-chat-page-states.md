...
| A4 | 未登录访问 | 无 session | **未决**(critique P2 登录墙,docs/archive/plans/2026-04-28-frontend-ssr-migration-todo.md)。占位方案:重定向 /login。倾向方案:允许 1 次试用回合后弹登录,本期不实现 | — |
...
- **右栏=橱窗,三层消化**(P2 双栏,2026-06-12 走查定):右栏永远 ≤6 张精选卡 + 分组折叠头 + 「すべて見る」入口,栏内 tab「一覧⇄マップ」;量分给地图(圏泡)与 SpotPicker(全量层)。禁止右栏长列表滚动——双栏会退化成"左对话右另一个 app"
- **Route 诞生 → 地图升格**(用户提议,2026-06-12):路线生成后所有地图视图切换身份——轨迹醒目绘制、pin 按步行顺序重编号、非路线点降透明、**金色路线 pill 常驻地图角落**(直达路线详情/歩くモード)。规则贯穿:右栏地图、GL 升格、歩くモード、公开分享页
- E1 双轨:V3 流追加新版本(流即历史)/ V1 右栏原位更新(面板即活文档)——同一数据,两种挂载点语义
- E2 旁路不演管线戏:没经过 agent 的操作不显示 agent 过程,只给再計算用时

**未决**:
- A4 登录墙(critique P2,产品决策,docs/archive/plans/2026-04-28-frontend-ssr-migration-todo.md 已记)
