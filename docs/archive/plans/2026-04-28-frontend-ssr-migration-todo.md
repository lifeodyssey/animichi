> Archived 2026-07-18: superseded by `docs/specs/2026-07-06-frontend-rebuild-spec.md`.
...
- [ ] **Evaluate TanStack Query for client data fetching** (raised 2026-06-05) — consider adopting `@tanstack/react-query` to replace the hand-written `fetch` + `useState`/`useEffect` loading/error/cache logic on the non-streaming pages (anime guide, routes, etc.). Gives caching, request dedup, retries, background refetch, mutations for free; integrates with Next.js SSR via `HydrationBoundary` for server prefetch. Additive + incremental (page by page), low risk; the project already uses `@tanstack/react-virtual` from the same family.
  - **Scope note:** this is TanStack Query (a data-layer library), NOT TanStack Start (the full framework / Next.js competitor). Do **not** migrate to TanStack Start — we just invested in Next.js SSR + OpenNext + Cloudflare; replacing the framework would throw that away.
  - **Decide first whether it's worth it:** the core UX is AI chat over SSE streaming, which is NOT Query's sweet spot (it handles request/response, not long-lived streams). The payoff is limited to how many non-streaming client-fetch pages exist. Audit `lib/api.ts` + page data flows before committing.
