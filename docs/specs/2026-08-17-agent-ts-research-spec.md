...
- 小结:重试基建差距 ~100 行 → ~50 行。**预期的最大成本项已不在重试基建,而在 eval 体系绑定**(617 case + pydantic evals 官方 runner + SD-30 双轴评估 + official-v1 八指标签核基线,2026-07-18 校准)。
...
3. **streaming**:SD-9 协议(AI SDK UI message stream)从 pydantic-ai `VercelAIAdapter` 切原生实现的等价性;typed output 渐进流验证。
4. **harness capabilities**:pydantic-ai-harness 的 capabilities / ManagedPrompt 的 TS 对应(含 CF codemode)。
...
8. **迁移成本**:~30.6k 行 Python、800+ 测试、7 工具,逐项估算。
