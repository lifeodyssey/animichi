...
> 设计原稿:`docs/archive/design-sync/`(claude.ai/design 项目导出,内嵌说明改名为 `design-project-log.md`)。
...
2. **覆盖每一张设计稿**:spec 必须含「设计稿 → story 覆盖矩阵」,`docs/archive/design-sync/` 顶层每个 .html 都要有归属(实现 story / 留档不实现 / 规范画布 / 索引画布),一张不落
...
依据 `docs/archive/design-sync/docs/user-journey.md` §2「5 段 18 断点」(J1–J18):
...
- **docs/archive/design-sync/docs/DESIGN.md**:動森キャンプ 90/8/2 三层配色(奶白棕/teal/金),永不纯黑纯蓝;按钮层级靠阴影深度非饱和度,3D 底影+pill+2px 边为核心识别;`Token Alignment Map` 预埋包↔应用 token 对照表与 CI 守护测试;Do/Don't 硬规则禁 `transition-all`/`space-y-*`/玻璃拟态/暗色模式/弹性缓动。
...
**Token 权威顺序**:`DS 补全 - Chat 桌面.html` > `docs/archive/design-sync/docs/DESIGN.md` > `skill/SKILL.md` > `_ds_manifest.json`/bundle(仅比对)。
...
| D8 | Neon/Supabase 拆分 | **SD-3 激活(2026-07-06,数据→Neon)+ SD-31 激活(2026-07-07,auth→Neon Auth)**:数据归 Neon;auth 后端亦迁 **Neon Auth**(激活 SD-3⑤),往全 Neon 收敛,渐进执行(见第七节 SD-3 / SD-31) | 原 06-23 设计被用户确认为终局路线;不再是 non-goal(auth 侧超越见 SD-31) |
...
| X17 | **iter15 旧前端功能 = TanStack 重建的行为参照清单(2026-07-07 用户裁决)**:主 repo `docs/frontend-rebuild-plan` 分支上 11 个 commit 不在重建基线(轮子二轮 B 节发现);处置 = docs 恢复两枚已 cherry-pick 进 feat/frontend-rebuild(19c9fa5;另一枚内容已存在),**前端功能 commits 不搬代码**(TanStack 全部重写)但其承载的行为必须逐项核对不丢:① magic-link 登录 modal + Start Exploring 规划转场(6921d10)② impeccable critique 修复:i18n 一致性/brand-warm 转场/登录打磨 + locale 检测接线(bcd3f05)③ locale switcher + 可跳过转场 + /search i18n + branded eyebrow + 具体化登录错误文案(1091341)④ iter15 分支系:clarification-redesign / location-prompt / nearby-chips / source-badges / i18n-cleanup / landing-rewrite。重建对应 story(S0.6 登录/S1.x chat/S5.5 首页)执行时以此清单 + 旧前端运行态为行为 oracle | 重建期参照;交付时逐项勾销 |
...
| SD-2 用户域访问 | **API-first 全走 /v1**:用户域 CRUD = workers 上新建 users 模块的 oRPC 路由(/v1/users/*),契约进 packages/contract;apps/web 的 **Neon Auth SDK** 仅用于 auth(**SD-31 — 原 supabase-js**);RLS 不作为访问路径;迭代 7 SDK/MCP 零改造复用同一契约 | 定案 |
| SD-3 数据面 | **Supabase 收缩为纯 auth,数据归 Neon**(激活 06-23 D8 设计并升级为终局):① selected_route 的 get_points_by_ids 改走 CatalogClient/Neon,消除同会话跨库混读(迭代 1 enabler,修 bug 性质);② 新建用户域表(路线/打卡/しおり/分享 token)一律生在 Neon,经 SD-1 工具链建表,JWT sub 衔接 auth;③ Supabase 中 catalog 域表(points/bangumi/aliases 等)冻结写入标废,稳定一迭代后删;④ 既有会话/消息/routes 数据迁 Neon 作为迭代 2-3 的独立 story(prod 数据量近零,一次性脚本);⑤ **auth 亦迁 Neon Auth——已由 SD-31 激活(2026-07-07),不再是 DD-1 冻结的远期项**:代码集成(登录 UI / JWKS 验签 / Neon RLS)落本列车 auth stories(S0.6/S1.7/S1.8 + workers/users JWT 验签);仍属 future wave 的尾巴 = 彻底退役 Supabase auth 项目 + 真实用户迁移(见 SD-31) | 定案 |
...
| eval 体系整备(SD-30,任务#13) | **定案(2026-07-06 用户批准;2026 agent eval 调研背书,对照表判定"骨架无过度工程+三缺口")**。背景新约束:主模型将走低价路线(DeepSeek Flash / MiMo 级),换模高频化,eval gate = 生命线。**两轴正交**:触发轴(何时跑)+ 维度轴(测什么=outcome/trajectory/system)。① **触发轴 L0-L3**:L0 冒烟(每 path×1 + P0 三语,~80 条;prompt/模型/守卫变更 PR 强制)/ L1 全量(617→~750;换模 gate/发版/夜跑)/ L2 对抗(25→~155:G-1 手写 30 + G-2 InjecAgent 子集 50 + G-3 AgentDojo 20 + 视觉注入 15 + 原对抗扩 40)/ L3 域专属(翻译 62→100、图搜 AB 100-150 对、路线误差 25、F 族 voice 30、judge 校准金标 50);总量 ~1,235(现 679),净增 ~560,飞轮1 逐步长不突击。CI 纪律:PR 只跑确定性+L0;flaky=跑 N 次聚合,**禁 retry-until-green**。② **维度轴补最大缺口**:617 集增 **trajectory 确定性断言**(expected_tools 集合/顺序、重复调用检测、步数上限;参考 DeepEval ToolCorrectnessMetric;纯确定性零 LLM 成本;先 L0 80 条打样,飞轮1 补全;遗留集 expected_steps 字段并入素材)——2026 共识:纯终态断言漏 corrupt success(终态对路径错),小模型退化最先坏中间决策链。system 维=Logfire span 已有。③ **统计门槛**:废"baseline−2点"裸阈值(95% CI 常见宽 3-5 点,n=25 上 2 点≈半个用例噪音)→ **分层 bootstrap 95% CI + paired 比较**(确定性断言下重采样零成本;小样本层 Clopper-Pearson;CI 重叠=不可判定→不拦截但标注)。④ **换模 gate runbook**:全量(outcome+trajectory)→ CI 对比 + score/円 报告(已获行业范式确认)→ 关键族 CI 下界不劣于 baseline → 人拍板。⑤ 存档迁 **Logfire Experiments**(sparkline/并排对比,已在用)替代手工 baselines/ 文件;自动 gate 判定 Logfire 无此功能,自建于 CI 脚本。⑥ **judge 纪律**:主链确定性不动;qwen3.5-9b 旁路=「明显跑偏」粗筛,不作多语言语义权威;未来需权威 judge(F 族)→ 先过 50 条人工金标校准(一致率 ≥80%/Kappa≥0.7)。⑦ 遗留集处置:v2(546)/runtime_journey(392)归档;plan_quality(163)/frontend_flows(10)取 expected_steps 后归档;intent_cases(33)并 L0 评估。⑧ 生产回流=飞轮1 已定(trace→case 脚本+eval_candidates 人审),不为内建回流换平台。⑨ **pydantic-evals 迁移 → DD-23 冻结**(2.5.0 Production/Stable、与 pydantic-ai/Logfire 同家打通;触发=轨迹断言+统计门控自建做重时)。**tier 字段死字段修复**:happy 510/edge 82→120/adversarial 25→155 由执行层真实消费。**spec 语言规范(同日用户指令)**:9 个 spec 文件 + seo-geo-plan.md = 英文(executor/reviewer 工程交付物);inputs/底档/conflicts/DD 登记册 = 中文(讨论台账) | 定案 |
| spec 收口三件(2026-07-07 用户批准) | **① P6 转定案**(全 spec 最后两个 [proposal] 之一):SSE 断线语义照既写方案——事件带 turn_id+seq、不做流内续传、断线后 GET 拉终态、生成中断 D4 卡带重试。**② 消息长度上限转定案**:MESSAGE_MAX_CHARS env,初值 4000(可调)。至此权威状态全绿,零 pending。**③ SD-28 双路由补记(规范审计 Top-3 + 轮子二轮合议,用户采纳)**:路线排序逻辑统一归 **TS catalog**(X12 方向)——selected_route 旁路从 Python route_optimizer 切到 deps.catalog,**与 SD-3① 跨库混读修复同一手术(归 S1.7,零额外排期)**;×1.3 绕路系数落点从 route_optimizer.py 改到 workers/catalog route.ts;Python route_optimizer.py(315 行)随切换退役;贪心 NN + union-find 算法本身不变(轮子二轮「不换有理」裁定,ST_ClusterDBSCAN 撤案)。**④ 重构发现归位**:F1(implement(catalogContract) 契约锁形 + 公网入参 zod 校验)+ 卫生批(F2 zombie 依赖/F3 reverse-geocoder 移组/F4 死代码/F5 asyncpg-stubs/F6 官方 DDG 工具 + 276KB 死 eval 数据集删除)→ **iter-0 新 story S0.10**;F7 aiohttp 栈死刑(~1,200 行)+ F8 OTel 双轨收敛 → 债卡排迭代 1-2(docs/archive/plans/2026-07-07-refactor-backlog.md) | 定案 |
| SD-31 认证后端(Neon Auth,激活 SD-3⑤) | **定案(2026-07-07 用户拍板,ADR 级传导)= 采用 Neon Auth(Better Auth v1.4.18 底座)为认证后端,退掉 Supabase auth-only,往「全 Neon」收敛**;正式激活并升级 SD-3⑤(原 DD-1 冻结:「远期 Neon Auth 成熟后 auth 亦迁、彻底退役 Supabase」)。**理由**:① 底座已从当初被否的 Stack Auth(不成熟)换成成熟的 **Better Auth v1.4.18**,当初否决理由过期;② auth 方式全覆盖——**magic-link 内建**(现产品在用)/ email OTP / Google·Apple·X 内建 / **LINE 走 Generic OAuth 插件**(与 Supabase 上自建 Edge Function 同等工作量,非障碍);③ **杀手锏 = 每个 Neon 分支自带独立 auth 环境** → multi-env/preview(per-PR/staging)的身份隔离难题直接消失(呼应 D10 多环境 + infra/pr-preview-env:Neon 分支自带独立身份+数据);④ auth 数据落 Neon(`neon_auth` schema,**RLS 原生**),**一个系统取代两个**;⑤ **已 enable**(Neon Auth `better_auth` 模式,2026-06-23 起);worker/agent 对其 **JWKS 验签**,JWKS 端点由 `NEON_AUTH_JWKS_URL`(env/secret 注入)提供——**端点主机名 / project-id 不入库**(公开 repo,一律走配置)。**代价**:Beta(GA 在望);仅 AWS 区(本项目部署区已覆盖 ✓)。**迁移边界**:provisioning 已 CLI 完成(`neonctl neon-auth`);**代码集成**(登录 UI 用 Neon Auth SDK、worker/agent 对 JWKS 验签、Neon RLS 策略)= **apps/web 重建时的迭代 story**(S0.6 登录 / S1.7 login-wall / S1.8 匿名 / S2.8·S3.7·S4.5 的 workers/users JWT 验签),**不改 legacy frontend**;Supabase auth 待集成落地后退役,有真实用户则迁移。**修订既有 SD(逐条)**:① SD-2 / SD-6 / G4 的「`supabase-js` 仅用于 auth」→「**Neon Auth SDK 仅用于 auth**」;workers/users 的 bearer JWT 从 Supabase 签发改为 **Neon Auth 签发 + 对 Neon Auth JWKS 验签**;② 激活 SD-3⑤(auth 亦迁 Neon),但 **SD-3②③④ 不变**(用户域表建 Neon、Supabase catalog 表冻结待删、会话/消息数据迁 Neon 照旧);③ 仍属 non-goal 的尾巴 = **彻底退役 Supabase auth 项目 + 真实用户迁移 + 删 Supabase catalog 表**(集成落地后的 future wave)。**iter-0 内已对齐**:iter-0 #252 域名 story(S0.8)的 auth callback/redirect + magic-link 模板配置,其 **auth 后端本次一并翻成 Neon Auth**(S0.8 清单第 5 项 / 域名收尾 AC / Files-changed)——因 S0.6 登录集成本身就是 iter-0 story,原「随 auth 集成 story 落地时迁」的说法不精确(该 story 即在 iter-0)。真正留给 #252 owner 的 manual-ops follow-up = **产品域名**迁移本身(animichi.com 上 Cloudflare、旧域 301、GSC/Bing 属性变更)这类注册商/DNS 运维,非 auth 后端配置 | 定案 |
