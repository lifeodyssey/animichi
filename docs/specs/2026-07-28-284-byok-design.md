...
- **Spec source of record**: `docs/specs/2026-07-06-frontend-rebuild/iter-1.md` §S1.11 (SD-11 + SD-20 + X3 + D18 + D5)
...
| Edge budget breaker | Latch checked **at the edge before forwarding** (`anonymousForward` → `budgetLatched`), authoritative check re-run in the container (`_budget_rejection`) | `worker/app.ts`, `worker/cost-breaker.ts`, `agent/interfaces/routes/chat.py` |
...
- **Files changed**:
  ...
  - `worker/rate-limiter.ts` (per-identity key derivation for authenticated identities)
  - `worker/byok.test.ts` / `worker/rate-limiter.test.ts`
- **AC**:
...
```bash
F=docs/specs/2026-07-28-284-byok-design.md
