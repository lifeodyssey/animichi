...
The CI route job calls the same validator for pull-request titles because GitHub uses that title as
the final main subject when the PR is squash-merged. Editing a title retriggers CI; no separate
workflow or required check is added.
