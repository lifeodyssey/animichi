...
| `workers/edge/` | edge | oxlint | `lint:oxlint` + `test:worker` (the package `test`: node:test suite, the contract projection conformance test, `test:bundle-smoke` — which bundles the pi kernel entrypoint and executes the artifact in workerd, #1246 — and `workers/edge/scripts/check-edge-ratelimit-namespace.sh`) + production-config `wrangler deploy --dry-run` |
