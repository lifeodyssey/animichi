...
| `SUPABASE_URL` · `SUPABASE_ANON_KEY` · `SUPABASE_DB_URL` | repo (no env override) | Auth today; **migrating to Neon Auth (SD-31)** | Edge worker, agent container, web build | Breaks login — `SUPABASE_URL` wrong → JWT verification 401s everything; `SUPABASE_DB_URL` wrong → the container fails its required-env check at boot (`buildContainerEnvVars` throws). `SUPABASE_ANON_KEY` is publishable by design |
