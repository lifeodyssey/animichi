...
2. **Worker-only anonymous chain** — `TURNSTILE_SECRET` and `ANON_ID_SECRET` use native
   ...
   DELETE the live copy. Because the value is generated rather than read from config, a later
   re-declaration rotates the seed (a removed state entry cannot reproduce the retained value) —
   so re-enabling anonymous access is another authorized rotation, not the value-preserving
   import the pre-#1676 config-backed seed allowed. Production currently disables anonymous
