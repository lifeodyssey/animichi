...
| `CLOUDFLARE_PULUMI_API_TOKEN` | `staging` + `production` environments | Pulumi 专用最小权限 token(R2:Edit + Zone DNS/Routes/Rulesets:Edit,zone 限 animichi.com;不含 Workers Scripts/Containers)— #674 最小权限分离 | `reusable-deploy-infra.yml` / `reusable-deploy-neon-secrets.yml` `pulumi up`（#1078 前仍从 GitHub 注入） | 与 wrangler token 分离即为轮换/爆炸半径隔离;轮换在 CF dashboard 原地 Edit 权限或 Roll 后更新两环境 secret |
...
| `PULUMI_BACKEND_URL` · `PULUMI_CONFIG_PASSPHRASE` | repo (**unreachable** — no non-environment-scoped caller) + `staging` + `production` | Pulumi DIY (R2) state URL and passphrase. Staging infra / neon-secrets now use Pulumi Cloud (#1077); these names remain for the production catalog `run_pulumi` path and the one-shot `migrate-pulumi-to-cloud.yml` import | `reusable-deploy-component.yml` (production catalog) and `migrate-pulumi-to-cloud.yml` | **Losing the passphrase makes remaining R2 state undecryptable.** Staging Cloud stacks use Pulumi Cloud encryption |
| `R2_ACCESS_KEY_ID` · `R2_SECRET_ACCESS_KEY` | repo (**unreachable** — no non-environment-scoped caller) + `staging` + `production` | R2 credentials for the leftover DIY Pulumi state bucket | `reusable-deploy-component.yml` (production catalog) and `migrate-pulumi-to-cloud.yml` | Wrong value → the remaining R2-backed `pulumi` path fails to authenticate |
