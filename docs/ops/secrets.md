...
does the same job for `ARCHITECTURE.md` against `workers/edge/auth.ts`.
...
| `CLOUDFLARE_PULUMI_API_TOKEN` | `staging` + `production` environments | Pulumi 专用最小权限 token(R2:Edit + Zone DNS/Routes/Rulesets:Edit,zone 限 animichi.com;不含 Workers Scripts/Containers)— #674 最小权限分离 | `_deploy-component.yml` 的 Pulumi state-backup 与 `pulumi up` 两步(仅 run_pulumi 的 catalog 部署 job) | 与 wrangler token 分离即为轮换/爆炸半径隔离;轮换在 CF dashboard 原地 Edit 权限或 Roll 后更新两环境 secret |
...
| `NEON_AUTH_JWKS_URL` | `staging` + `production` only (no repo-level fallback) | Neon Auth (Better Auth) JWKS endpoint for the dual-issuer JWT path | Worker edge (`workers/edge/auth.ts`), gated by `NEON_AUTH_ENABLED` | Currently low-blast-radius: Neon Auth is provisioned but not yet the active issuer (SD-31), and tokens route by `alg`/`iss`, so a wrong JWKS URL would only affect a Neon-issued token if one ever arrives — unverified in production today since no live flow mints one yet |
| `POST_DEPLOY_DIAG_TOKEN` | `staging` + `production` only (no repo-level fallback). **⚠️ NOT YET PROVISIONED (issue #709) — this PR only wires the plumbing; the owner must `gh secret set POST_DEPLOY_DIAG_TOKEN` for both environments separately.** Until that happens, the post-deploy `auth-config-check` step fails with "expected 200, got 404" — that looks like a broken route, not a missing secret, so check here first | Shared bearer secret gating the post-deploy diagnostic route `GET /internal/auth-config` (issue #709). **Not a user-facing credential** — it proves nothing except "this request came from the post-deploy job", and the route it gates only ever returns two booleans (`neonAuthEnabled`, `jwksIssuerMatch`), never a URL or any other secret value | GitHub environment secret → pushed to the Worker by `wrangler-action`'s bulk `secrets:` upload in `_deploy-component.yml`'s "Deploy Worker" step (via the `worker_secrets` list — same shape as `NEON_AUTH_JWKS_URL` above, not the `post_deploy_secrets` chain) → verified by `workers/edge/authConfigCheck.ts`'s `isDiagAuthorized` (constant-time compare; an unset token denies every request, fail-closed) → also held by `.github/scripts/post-deploy-assert.sh`'s `auth-config-check` subcommand, which sends it as `Authorization: Bearer …` | Safe to rotate any time: `isDiagAuthorized` fails closed on any mismatch (old value, new value, or unset), so a stale value on either side only ever 404s the diagnostic route — it can never fail open. Update the environment secret, then re-run the post-deploy suite to confirm the new value reached the Worker |
...
| `GEMINI_API_KEY` | Was Live (this table, above) until #656 (2026-08-04): photo-search recognition now rides the main agent's multimodal input (`apps/agent/agent/agents/photo_vision.py`) instead of the standalone `GeminiVisionProvider`, so nothing in `CONTAINER_ENV_KEYS`, `wrangler.toml`, or any workflow reads this name anymore | `gh secret delete GEMINI_API_KEY` once the deploy carrying #656 is confirmed live in production — no dependency to check first, the code path it fed no longer exists |
