...
| `AGENT_DATABASE_URL` | `staging` + `production` environment secrets only | Agent-domain Neon DSN for scheduled retention | Same name in `workers/maintenance/wrangler.toml`, `_deploy-component.yml`/`ci.yml`/`deploy.yml`, and `workers/maintenance/src/index.ts`; it is uploaded only to the maintenance Worker | Missing or wrong → the Worker deployment or scheduled query fails loudly; quota/session rows are retained rather than silently deleted from another data domain. Rotate one environment at a time and verify its next Cron Trigger Past Event |
