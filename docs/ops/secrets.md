...
| `ZEN_GO_API_KEY` | repo (no env override) | **Production LLM gateway.** MiMo `mimo-v2.5` is routed through the zen/go gateway (`https://opencode.ai/zen/go/v1`) | Agent container, `ci.yml` eval smoke, `agent-eval-nightly.yml` | Breaks every chat turn and the eval lanes: the provider call 401/403s and the user sees the agent's generic failure response, never the raw provider error (SD-19 forbids surfacing upstream text) |
...
| `NEON_AUTH_JWKS_URL` | `staging` + `production` only (no repo-level fallback) | Neon Auth (Better Auth) branch JWKS — the edge's **ONLY** identity source (AUTH-2 #950) | Staging edge Worker reads a checked-in var (`[env.staging.vars]`, placeholder until the real issuer); the GitHub secret is still uploaded to the **users** Worker (staging + prod) but is now a **dead binding** — users trusts only the edge-forwarded identity and no longer verifies JWTs, so it can be dropped from `worker_secrets` at the next deploy-touch | Wrong value → every Neon-issued bearer 401s on staging; production edge fails closed on any bearer (JWKS unset) until its branch is provisioned |
| `SUPABASE_URL` · `SUPABASE_ANON_KEY` | repo (no env override) | **Retired** — the auth plane is Neon Auth (AUTH-2 #950 hard cut); nothing reads these for auth anymore | No consumer (edge verifies Neon JWKS only; web logs in via the Better Auth client) | **Dead** — delete from the repo and any deploy secret list. `SUPABASE_DB_URL` (agent data plane, below) is a separate, still-live secret despite the shared prefix |
| `SUPABASE_DB_URL` | repo (no env override) | **Retired** — the Agent runtime's data-plane DSN is `AGENT_SVC_DATABASE_URL` only (#995); the settings fallback was deleted. Still uploaded to the edge Worker (its `CONTAINER_REQUIRED_KEYS` list still names it) and forwarded, but the container ignores it | No consumer (container settings no longer read it) | **Dead for the agent** — remove from `container-env.ts`'s required list and the deploy secret lists once the edge cutover lands; delete the GitHub secret after that |
| `SUPABASE_SERVICE_ROLE_KEY` | repo + `staging` + `production` | Legacy Supabase service-role credential | **No code reads it since AUTH-1 (#945)** removed the `sk_*`/api_keys edge path; still passed through the root Worker secret list and the SAFE-1 manifest (awaiting the manifest re-pin cleanup). Wrong value has no effect today |
