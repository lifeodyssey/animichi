...
`NEON_DATABASE_URL`, and `LOGFIRE_TOKEN` are referenced inside environment-scoped jobs, so the
...
| `CLOUDFLARE_PULUMI_API_TOKEN` | `staging` + `production` environments | Pulumi 专用最小权限 token(R2:Edit + Zone DNS/Routes/Rulesets:Edit,zone 限 animichi.com;不含 Workers Scripts/Containers)— #674 最小权限分离 | `cd.yml` staging/production `pulumi up` (exported into the apply as `CLOUDFLARE_API_TOKEN`) | 与 wrangler token 分离即为轮换/爆炸半径隔离;轮换在 CF dashboard 原地 Edit 权限或 Roll 后更新两环境 secret |
