...
| `R2_TILES_ACCESS_KEY_ID` · `R2_TILES_SECRET_ACCESS_KEY` | Local operator `.env` today (not yet a GitHub secret); intended `staging` + `production` GitHub environments once the script moves to CI | Dedicated S3-compatible R2 credentials scoped to the `map-tiles`/`map-tiles-staging` buckets — tile supply uploads (the `R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY` pair above cannot write here) | `scripts/build-pmtiles.sh upload` (`aws s3 cp`/`sync`, multipart); read via `env_value()`, env first then a grep of repo-root `.env` | Wrong or missing value → `aws s3 cp`/`sync` fails closed with a clear message naming both vars before any network call |
