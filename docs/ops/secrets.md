...
| `NEON_AUTH_JWKS_URL` | `staging` + `production` only (no repo-level fallback) | Neon Auth (Better Auth) branch JWKS — the edge's **ONLY** identity source (AUTH-2 #950) | Staging edge Worker reads a checked-in var (`[env.staging.vars]`, placeholder until the real issuer); #1047 dropped this value from the routine users-Worker deploy path (`ci.yml`/`deploy.yml` `worker_secrets`), so the GitHub environment secret is now a **dead binding** — users trusts only the edge-forwarded identity and no longer verifies JWTs. The only remaining reference was the break-glass `staging-cutover.yml` cutover-phase-e-deploy-users job, removed along with this fix | Wrong value → every Neon-issued bearer 401s on staging; production edge fails closed on any bearer (JWKS unset) until its branch is provisioned |
