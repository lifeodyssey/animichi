...
| `SUPABASE_DB_URL` | repo (no env override) | Agent-domain Postgres connection string (asyncpg) — the legacy data plane, superseded on staging by `AGENT_SVC_DATABASE_URL` (#912 follow-up) | Agent container | Wrong value → the container fails its required-env check at boot (`buildContainerEnvVars` throws) |
