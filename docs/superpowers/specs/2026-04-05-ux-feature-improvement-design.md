...
**Status:** PARTIALLY DONE (90% — only root cleanup scraps + ResultPanel auto-open remain)

> **Update (2026-04-11):** Phases 1-5b are fully landed. Phase 0 is mostly done (legacy dirs removed, `.gitignore` updated) but `coverage.xml`, `htmlcov/`, `findings.md`, `progress.md`, `task_plan.md` still in root. Phase 5c is partially done (InlineSummaryCard with thumbnails and onActivate landed in MessageBubble.tsx, but auto-open in AppShell.tsx line 168 is still present). See "Remaining Work" at bottom.
...
## ✅ Phase 0: Housekeeping (gstack + root cleanup + branch cleanup) (partially landed)

### ✅ 0a. Install gstack (landed)
```bash
...
## ✅ Phase 1: Remove 20-point limit + Geocoding (Backend) (landed)

### ✅ 1a. Remove hardcoded limit (landed)
...
### ✅ 1b. Google Geocoding gateway (landed)
...
### ✅ 2d. Public API (landed)

- `backend/interfaces/public_api.py`: Add `"clarify": "Clarification"` to `_UI_MAP` (note: mapping was implemented via executor_agent.py intent output instead of `_UI_MAP`)
...
## ✅ Phase 4: shadcn/ui + 京アニ Design (Frontend) (landed)
...
## Phase 5: Frontend Features (depends on Phase 4) (mostly landed)

### ✅ 5a. Multi-dimensional point grouping (episode + area) (landed)
...
### ✅ 5b. Route info panel fix (landed)
...
### 5c. ResultPanel click-to-open (partial — inline card landed, auto-open still fires)
...
1. **Phase 0b (root cleanup scraps):** Delete `coverage.xml`, `htmlcov/` from root. Move `findings.md`, `progress.md`, `task_plan.md` under `docs/iterations/iter5/`.
2. **Phase 5c (ResultPanel auto-open):** Remove `latestVisualResponseMessage` fallback in `AppShell.tsx` so ResultPanel only opens on explicit user click, not automatically on every visual response.
