...
| SD-26 管线补章:图搜 RAG 管线(D1-D3,任务#12) | **定案(2026-07-06 用户批准)**。**D1 编排**:阶段1 = vision 认作品,**实现基线 = 独立 vision 调用**(不赌主循环模型多模态;DeepSeek V4 Pro vision 能力[待核实],若具备则省略独立调用作为优化路径);阶段2 = `match_scene` **单工具内部确定性管线**(粗筛+精排一口气,agent 只见一个工具,SD-7 不加 LLM 编排层哲学);工具签名 `match_scene(image, scope: {bangumi_ids} | {geo, radius})` 统一作品域/GPS 反向发现两场景,后半段管线复用。**D2 索引管线归 catalog**(X12 数据资产归数据 owner):Anitabi 同步管线顺带算 embedding(Gemini Emb 2 batch,首次全量 ~$5),halfvec(1536) 存 Neon 点位表,source 标记 anime_frame / user_checkin(飞轮3 打卡照人审通过后入库,逐点解锁 real2real);agent 只消费不生产。**D3 match_scene = 本项目首个「内含 LLM 调用的工具」**(显式突破补章 II"工具=纯代码"默认),三纪律:① 成本入 daily_usage(SD-18 钩);② key 策略 = 精排走 BYOK 用户 key(有则用,三族全有 vision)否则平台 vision 模型,embedding 永远系统 key;③ 单查询精排 **≤2 批**上限(防粗筛失灵成本毛刺),超限返回粗筛 top3 带降级标注 | 定案 |
