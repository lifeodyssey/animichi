...
| 路程规划(SD-28,任务#11) | **部分定案(2026-07-06 用户逐层确认)**。现状基线核实:`backend/agents/route_optimizer.py` 纯函数零 I/O 零第三方 = union-find 50m 聚合 → 贪心最近邻 → 拍照数×节奏停留倍率(chill/normal/packed) → **haversine÷80m/min 估步行**;上限 50 簇。核心短板=haversine 直线距离(非真实沿街/换乘)。**分层方案**:层0 haversine 启发式(保留=无路网兜底+离线 Walk 用);**层1 步行段=自托管 OSRM/Valhalla + OSM(Geofabrik 日本 extract,ODbL 免费),MVP 就上**(零经常成本、无 ToS 缓存限制、补掉最大失真);层2 交通段(跨簇>阈值触发)=**待定**(用户倾向"不贵就接 Google Directions/Routes API",但**生死线=Google Maps ToS 能否缓存 transit 结果进自有 DB**——若禁则与 SEO 分享页持久化路线直接冲突,须调研核实价格+缓存条款再拍;备选=自托管 OpenTripPlanner+GTFS-JP / 纯深链外链兜底 / 换乘案内类 app 公开 API·数据 / 已用地图栈自带路由 API)。层3 真实换乘=future→DD 冻结(触发=交通估算不准的用户信号量)。**已定基座(用户确认)**:① 步行自托管开源栈不算"第三方 API 依赖";② 不追求最优 TSP,贪心最近邻够好(一日游≤50 点误差无感);③ 节奏模型沿用现有停留倍率;④ 离线 Walk(G6/迭代3)回落 haversine,层1 是在线增强。**静态数据资产**[有据]:OSM 路网(Geofabrik/ODbL)、铁道线路+车站(国土数値情報 N02/政府开放)、GTFS-JP(覆盖碎片化慎用)。**待办**:换乘调研代理(Google 日本 transit 覆盖+2026 价格+缓存条款+GitHub 现成换乘方案+换乘案内 app API/公开数据+深链兜底)返回→拍层2→SD-28 全定案 | 部分定案(层2 待调研) |
