...
| 图片搜索两阶段(SD-26,任务#7) | **定案(2026-07-06 用户确认)**:文本 RAG 不引入(结构化数据走 SQL/PostGIS 精确查询,套向量=负优化);图片搜索=视觉问题,野心边界=认到具体场景/机位。**两阶段架构**:阶段1 LLM vision 粗筛认作品(零索引,借 LLM 动漫世界知识白嫖一级检索,搜索空间几万点→单作品几十点,三个数量级)→ 阶段2 作品内精匹配机位(候选已压到几十,pgvector on Neon 足够)。独家资产 = Anitabi 每点位「动画帧+现实照片」配对(跨域匹配天然参考对,用户图可双路匹配取最优)+ 飞轮3 打卡照回流越用越准。**分层排期**:阶段1 进迭代1(chat「写真」态入口,认出作品名走现有 resolve_anime;认不出冷门作品→clarify 追问降级,不新增机制;单独 releasable = 拍图→认番→出巡礼地图);阶段2 进迭代4(与対比図/迭代3 Walk 机位共享参考图数据管线,顺路建索引边际成本最低)。**待定**:阶段2 embedding 选型(多模态 LLM embedding / CLIP·DINOv2 / 动漫专用模型 / 或 LLM vision 直接排序免 embedding)——2026 跨域/动漫图搜调研代理进行中,回来后拍板 | 定案(embedding 选型待调研) |
