...
| G4 | Backend ownership | **Full-stack vertical slices**: any story needing new backend capability brings its own minimal enabler. Final ownership rule: catalog-domain data → `workers/catalog` oRPC; **user-domain data → a new TS service `workers/users`'s oRPC routes (`/v1/users/*`), backed by Neon+Drizzle, contract in `packages/contract`**; `apps/web`'s `supabase-js` is **auth-only** and never talks directly to any data table; **no new data endpoints may be added to the agent service** — the agent keeps only session/orchestration state (single-tool loop + deterministic bypass, SD-7). **X11 discipline**: `apps/web` itself must always consume the capability/user-data surface through the public `/v1` API, never a private backdoor; Iteration 7's SDK/MCP reuse the same contract and expose **task-shaped capability only** (SD-12), never a chat pass-through |
...
| D8 | Neon/Supabase split | **SD-3 activated, progressive rollout** | Supabase narrows to pure auth, the data plane moves to Neon, 5 sub-points (① fix the selected_route bug / ② build new tables in Neon / ③ freeze the Supabase catalog tables / ④ migrate sessions etc. / ⑤ the eventual Auth migration is out of scope for this train) |
...
### D16-D19 (from scattered points in inputs §9 "model strategy"/"subagent, final ruling"; **per the C6 ruling (Patch A, 2026-07-06 evening), D16/D18/D19 are now all Finalized**: D17 was already promoted to Finalized after being walked through in §10 Step 8/SD-24; D16 and D19 are confirmed as pre-existing finalized defaults and are removed from the legacy [proposal, pending confirmation] list per C6c; D18 is made explicit per C6b, grounded in what SD-20's D18-boundary regression AC already implied)

| # | Decision | Status | Content |
|---|---|---|---|
| D16 | Model-switch gate | **Finalized (per C6c ruling — confirmed as a pre-existing default)** | Switching the primary model must pass the eval gate (the 617-case suite, weighted toward locale+intent, decided on a combined score/¥ basis); shares infrastructure with the X8 layered eval gate. The concrete model-switch gate runbook is SD-30④ (full outcome+trajectory eval → CI comparison + a score/¥ report → key-family CI floor no worse than baseline → human sign-off) |
...
| X13 | Migration toolchain — superseded by SD-1 (dual chain + atlas-provider-drizzle) | See SD-1 |
| X14 | Edge worker graduation — corrected by SD-6 (already TS + 16 test cases, the only gap is CI) | S0.3 |
...
| SD-1 Migration chain | Dual chain + atlas-provider-drizzle; boundary written up in `docs/ops/migrations.md` (S0.9) |
| SD-2 User-domain access | API-first, `/v1/users/*` oRPC (`workers/users`); `supabase-js` is auth-only |
| SD-3 Data plane | Supabase narrows to pure auth, data plane moves to Neon (5 sub-points, D8); ⑤ the eventual Auth migration → DD-1 frozen |
...
| SD-6 Edge worker | Already TS + 16 test cases; the only gap is wiring CI (S0.3) |
...
- `apps/agent/pyproject.toml` line 46 declares `pydantic-ai-guardrails>=0.2.2`, with zero imports anywhere in the repo — **confirmed dead dependency** (this finding came from the Planner's own code audit and isn't affected by the P-series "proposal under discussion" status; see S1.6/S1.12 for how it's handled).
- `/v1/chat` currently uses `VercelAIAdapter` — **corrected by Backfill A (2026-07-06 evening)**: an earlier version of this doc flagged this as "not quite matching the SD-9 three-event naming," a risk item. Since SD-9 has been revised so that the AI SDK UI message-stream protocol itself *is* `VercelAIAdapter`, the two are now aligned — this is no longer a risk needing reconciliation, it's simply the existing correct implementation. S1.1/S1.2's job becomes "confirm the existing `/v1/chat` endpoint matches the revised SD-9's semantic mapping and fill in the custom data-parts schema," not "reconcile protocol ownership."

### Global convention: platform capability adaptation layer (X10, Finalized)

`apps/web/src/platform/` establishes a thin interface layer wrapping camera / geo / haptics / wake-lock / clipboard-share; **product components may never call `navigator.*` directly**. Any story touching camera/location+vibration/clipboard/share requires an AC of "goes through the adaptation layer" `-> unit`.

### Global convention: user-domain data access path (SD-2, Finalized)

A user's own data is always accessed through the **newly built `workers/users` TS service**'s oRPC routes (`/v1/users/*`), landing in **Neon**, with the contract in `packages/contract`. `apps/web`'s `supabase-js` is **auth-only**.

### Global convention: Chat streaming protocol (**the revised SD-9, Finalized, backfilled from inputs §10 Step 2, superseding this section's earlier "three-event SSE" wording** + the P6 proposal-pending detail)

**The Finalized portion**: the protocol is the AI SDK UI message stream, backend pydantic-ai's official `VercelAIAdapter` (`/v1/chat` is already this implementation today), frontend AI SDK v7 `useChat` (inside TanStack Start). Semantic mapping: step badges ← the tool-parts state machine; progressive cards ← data parts overwritten in place by ID; the waiting ritual/fox mood ← derived by a frontend state machine (not a field the protocol carries natively); final state ← the `finish` event. The project's own contract narrows to **a zod schema for custom data parts** (in `packages/contract`), no longer a standalone three-event schema. Generative registry components are designed partial-tolerant from day one (carrying forward SD-13 rule 3); the protocol must be able to degrade gracefully; the discriminated union's `intent` field must arrive ahead of the other fields (this constraint carries over unaffected by the protocol change). Iteration 1 spike: confirm whether typed output can stream out progressively as data parts through `VercelAIAdapter`; if not, the backend pushes data parts actively between tool calls instead. Standardize on `/v1/chat`; the existing custom-SSE `/v1/runtime/stream` retires once the chat migration completes, leaving no dual protocol.

**The [proposal, pending confirmation] portion (P6, inputs §8, now restated in AI SDK protocol terms)**: custom data parts additionally carry `turn_id`+`seq`; **no resume-on-disconnect support** (no SSE Last-Event-ID–style resumption — a run can't be picked back up mid-way; AI SDK's native resume capability is preferred wherever it can be reused directly rather than building our own); after a disconnect, the client calls `GET messages` to fetch the final state rather than attempting to resume the stream; a generation interruption shows in the UI as a D-series exception-state card (D4). This detailed mechanism is already written into the ACs for S1.1/S1.2/S1.6, but flagged as pending user confirmation; if the user doesn't accept the "no resume-on-disconnect" trade-off, the Coordinator needs to align with the user during review before finalizing it.

### Global convention: the Generative UI constitution (**Finalized — the core philosophy backfilled from SD-13; the URL allowlist clause is likewise Finalized per the C6a ruling**)

- **Finalized (SD-13, Philosophy A)**: the LLM can **only** pick an already-registered component type from the registry + fill in a structured semantic payload; the app's own registry handles rendering, and the LLM **never generates UI code**. Three rules: ① an append-only card stream; ② additive-only versioning (governance modeled on MCP's deprecation policy: Active/Deprecated/Removed, ≥12 months); ③ partial-tolerant components (may omit fields + a skeleton slot; Storybook builds half-populated and legacy-payload stories). `presentation_hint` = a server-suggested value + the frontend has final say + unknown values degrade gracefully.
- **Finalized (per the C6a ruling, grounded in SD-19's same-origin injection-defense requirement)**: any URL appearing inside a payload **may only** render from a catalog data source or an explicit allowlist source. This clause is the rendering-layer, same-origin corollary of SD-19's injection-defense architectural invariant (external content must be delimited and treated as untrusted) — SD-13's registry philosophy together with SD-19's same-origin requirement already cover this clause jointly, so no additional dedicated user confirmation is needed.
- This clause is written into the Reviewer checklist for the Iteration 1/6 component stories as a hard merge-blocking gate.

### Global convention: security/privacy hard-item overview (**updated by Backfill A, 2026-07-06 evening**: P2/P8/P9/P10 were each promoted to Finalized after being walked through in the subsequent §10 rounds, P3 has been rescinded and superseded, leaving only P6 and the message-length cap as [proposal, pending confirmation]; quick-reference table)

| Hard item | One-liner | Main landing spot | Status |
|---|---|---|---|
| P2 | External content (web_search, etc.) entering context must be delimited + marked untrusted | S1.12 | **Finalized (backfilled from SD-19)** — `<untrusted_web_result>` delimiting + a system-prompt statement + `detect_prompt_injection` extended to cover tool-returned content + an architectural invariant (tools are read-only + results can never be promoted to instructions) + source tiering |
| P3 | ~~A tool-boundary timing + token-cost-accumulation middleware, feeding the X4 circuit breaker~~ | S1.12 | **Rescinded, superseded by SD-18's usage-metering hook** — the original timing portion is dropped (the Logfire span already covers it, avoiding duplicate build), replaced by `result.usage()` → the `daily_usage` table + a container-ingress circuit breaker |
| P6 | SSE events carry turn_id+seq, no resume on disconnect, fetch final state after a disconnect | S1.1/S1.2/S1.6 | **[proposal, pending confirmation]** (now restated in terms of the revised SD-9's AI SDK protocol, see the "Global convention: Chat streaming protocol" section) |
| P8 | Outbound-request SSRF guard (https-only + block private-net/loopback/metadata IPs) | S1.11 | **Finalized (backfilled from SD-20)** — the strict variant with post-resolution IP validation (no re-resolving, preventing TOCTOU/DNS rebinding; no auto-following redirects), no domain allowlist (an allowlist would kill the core BYOK use case of self-hosted vLLM/relay providers) |
| P9 | Precise GPS never enters Logfire, truncated to ~100 m, storage layer keeps full precision | S3.3 | **Finalized (backfilled from SD-21)** — coordinates are cut to 3 decimal places before entering trace; implemented as a side effect of the header-allowlist middleware |
| Message-length cap | A cap on the length/type of user input messages (a Guardrails addendum) | S1.12 | [proposal, pending confirmation] (not covered by any round in §10/§11) |
...
**Guidance for the Coordinator**: the Finalized entries above can serve directly as hard merge-blocking gates; **only P6 and the message-length cap** still need a review-stage pass with the user — once confirmed, flip the status from "[proposal, pending confirmation]" to "Finalized," or adjust/drop the corresponding AC per the user's call. Until the user has taken a clear position on these two items, the Reviewer should not treat them as "must-pass-to-merge" hard gates.
...
- D7 (REJECTED), D9/D10 (non-goal), SD-3⑤, and actually deleting the Supabase catalog tables — none of these are touched in this train.
...
- **Planner's supplementary defaults (Finalized, this spec's own call)**:
  ...
  5. The `pydantic-ai-guardrails` dead dependency gets a binary choice (wire it up or remove it); the Planner recommends removal — this came from the Planner's own code audit, independent of the "[proposal, pending confirmation]" tier below, and needs no further user confirmation.
...
### Defaults set by the main session (the [proposal, pending confirmation] portion, scattered points in inputs §8; **updated by Backfill A**: the MCP client/internal skill framework have moved up to "the Finalized portion" above; what's left here is only the scattered points not covered by any subsequent round)
...
|---|---|---|---|---|---|
| 0 | Foundations | 9 | Fully detailed | The apps/web skeleton + deploy-chain fixes (worker/** CI wiring) + the map ADR + layered eval + the DS foundation + spike code migration + old-frontend removal + SEO foundations (**backfilled from SD-27**: the full animichi.com domain-migration checklist + a static robots.txt/llms.txt page + a sitemap skeleton + an IndexNow key + GSC/Bing/CF Analytics + Organization/WebSite/BreadcrumbList + a hard AC for CF crawler reachability, see `2026-07-06-seo-geo-plan.md` §7) + doc rewrites (`docs/ops/migrations.md`) | `2026-07-06-frontend-rebuild/iter-0.md` |
| 1 | 計画 (Plan): Chat | **12** | Fully detailed | Chat Phase 1's full single-column-stream state set (44 states) + the **AI SDK UI message-stream protocol** (the revised SD-9, Finalized, superseding the old "three-event SSE" wording, P6's disconnect semantics still a proposal detail) + opening up anonymous use/quotas/Turnstile/BYOK's three provider families (SD-11 Finalized, each its own story) + the P5 login wall + a hard first-token SLO + the `selected_route` cross-database bug fix + **hardened agent guardrails** (S1.12, including SD-19 injection defense [Finalized] + SD-18's two new hooks [Finalized] + the message-length cap [still proposal-pending] + paying down the guardrails dead dependency) + image-search phase 1 (SD-26, the chat "photo" state, LLM-vision coarse screen to identify the anime) + full instrumentation (the fuel tank for flywheels 1-5 + the five image-search signals, SD-22/23/26) | `iter-1.md` |
| 2 | 承接 (Follow-through): details + list | 9 | Fully detailed | Route details v2 + the マイルート (My Route) bookshelf view + the save/list enabler (`workers/users` oRPC + Neon) + sessions data migration; the anime page's fact ledger (SD-15) fields accumulate alongside chat |
| 3 | 歩く (Walk) | 9 | Detailed before kickoff | The Graduation transition + Walk's 10 states + offline all-at-once + the check-in enabler (`workers/users` oRPC + Neon, including **SD-21's Finalized** GPS truncation: cut to 3 decimal places/~100 m before trace, full precision in storage) + the fox trot sprite + conversation_messages data migration | `iter-3.md` |
| 4 | 残す (Keep): しおり | 7 | Detailed before kickoff | The しおり (keepsake booklet) layout family (a client-side image pipeline, EXIF stripped by default) + `/s/:id` public sharing (SSR, dynamic OG backfilled from SD-27) + 対比図 (comparison-diagram) creation (client-side canvas) + R2 upload + image-search phase 2 (SD-26, precise scene/shot-angle matching within an anime: embedding coarse screen + LLM-vision fine ranking) + a 対比図 image sitemap (SD-27) | `iter-4.md` |
| 5 | 発見 (Discover): anime pages + homepage | 8 | Detailed before kickoff | The anime public page, variant A, encyclopedia-style (SSR) + new public catalog oRPC routes + the data-quality gate (X15) + the homepage (続きから depends only on the sessions/routes lists) + programmatic SEO/GEO (**backfilled from SD-27**: trilingual fact-summary blocks + narrowed JSON-LD + region pages + a new-season sitemap SLA + the quality gate wired into CI, see `2026-07-06-seo-geo-plan.md` §7) + flywheel 2 (intent taste) and flywheel 4 (SEO growth) begin analysis (SD-22/23) | `iter-5.md` |
...
| 7 | Open API | 7 | Detailed before kickoff | Unlock the eval gate + **external release order per SD-25**: the agent skill (first) + a quick, coarse-grained MCP server (**A2A deferred, → DD-10 frozen, not done in this iteration**) + MCP (Finalized per SD-12: task-shaped capability, a Workers thin adapter) + publish the OpenAPI spec + an npm SDK + the Python client graduates + a minimal MCP Apps subset (SD-13) + MCP-as-GEO submissions (SD-27: MCP Registry/mcp.so/Glama/an isitagentready self-check) + llms.txt gets an MCP-endpoint addendum (**no llms-full.txt, backfilled from SD-27's non-goal list, superseding the old version's mention of llms-full.txt here**) | `iter-7.md` |
...
**Note on story counts exceeding the "3-8" guideline**: Iteration 0 (9) / Iteration 1 (12 — the anonymous-use work split four ways + the 44-state set + the new S1.12 guardrails story) / Iterations 2 and 3 (9 each, data migration is its own story) / Iteration 7 (7, the SDK/MCP work expands) — all have been individually verified as "completable by a single executor in a day."
...
5. Visuals match the corresponding canvas — the Tester screenshot-compares against the design source, state by state.
6. Trilingual (ja/zh/en) copy is complete.

**Note**: ACs tagged [proposal, pending confirmation] (now only P6, the message-length cap; D16/D18/D19 and the Generative UI constitution's URL allowlist clause have all been promoted to Finalized by the C6 ruling and no longer belong on this list) are not a blocking condition for Releasable status until the user confirms them — i.e., a story can still merge/ship once its other (Finalized) ACs all pass; the pending items get filled in or adjusted once the Coordinator has the user's answer, without holding the whole story hostage.
...
- **X8**: any change to the agent's behavior surface requires the 5-case smoke PR gate to pass plus a clean run of the full 617-case nightly suite.
...
**[proposal, pending confirmation] DoD addenda** (recorded as "already written into the AC, pending activation" until the user confirms, not part of the mandatory gate): P6's disconnect semantics and the message-length cap.
...
- **SD-3⑤'s eventual Neon Auth migration** (**→ DD-1 frozen**), **actually deleting the Supabase catalog tables**: out of scope for this train.
...
- **An internal skill framework, an mcp-client (consuming third-party MCP)**: both already Finalized as non-goals via SD-24 (**→ DD-9/DD-8 frozen**), not [proposal, pending confirmation].
...
**Formally adopted as spec**: `ds-審査.md` (ds-审计.md) already ruled that "departure time + location are mandatory questions"; C2t is the concrete implementation of that ruling, not a standalone proposal. Lands in S1.3.
...
### 8.8 This spec's [proposal, pending confirmation] list (needs the Coordinator to walk the user through it item by item; **updated by Patch A per the C6 ruling — the scope has narrowed further**)

**Updated by Patch A (2026-07-06 evening)**: per the C6 ruling, D16/D18/D19 (in the D16-D19 table) and the Generative UI constitution's URL allowlist clause have all been promoted to Finalized. What remains [proposal, pending confirmation] in this spec is now down to exactly two items: P6 (in the security/privacy quick-reference table) and the message-length cap. (Previously, per Backfill A's evening update, this list also carried D16/D18/D19 and the URL allowlist clause — that update is now superseded by the C6 ruling above.) These two remaining items are **not** "unanswered open design questions" (unlike the 8.1/8.2 scenario, where the Planner supplies a recommended default) — they are cases where **a complete plan already exists, but the process skipped the user-discussion step** and the main session flagged this. The right move is to "confirm or adjust," not "design from scratch."
...
| **A [proposal, pending confirmation] item mistakenly treated as Finalized and acted on** (**per the C6 ruling, the scope has narrowed to just P6/the message-length cap**; D16/D18/D19 and the Generative UI constitution's URL allowlist clause have all been promoted to Finalized and no longer fall under this risk): if the Coordinator/Executor skips the user sign-off step and treats a remaining pending item as a must-pass hard gate for a PR | Replays the same "moved forward without discussion" problem, wasting rework | The Coordinator must complete one round of user sign-off before scheduling, flipping the status from "[proposal, pending confirmation]" to "Finalized," "adjusted," or "removed," before execution begins | S1.1, S1.2, S1.6 (P6), S1.12 (the message-length cap) |
| **If the user overturns P6's "no resume on disconnect"** | The ACs already written into S1.1/S1.2/S1.6 around "no resume + fetch final state via GET messages" would need reworking into a resume-capable alternative | Keep the current AC as the default, noted as pending user confirmation; the rework scope is limited to these few stories and doesn't touch the revised SD-9 protocol itself (the AI SDK UI message-stream portion stays Finalized regardless) | S1.1, S1.2, S1.6 |
...
- **Updated by Backfill A**: the relevant ACs for S1.11 (BYOK/P8), S1.12 (guardrails/P2), and S3.3 (Walk/P9) have all been promoted to Finalized (SD-20/SD-19/SD-21) and are no longer pending items. What's left needing an explicit Coordinator-planned "user sign-off" action is only S1.1/S1.2/S1.6 (P6) and S1.12 (the message-length cap).
...
- The Reviewer checks `ac_total == ac_with_test` and a Codecov patch ≥95%; for ACs tagged [proposal, pending confirmation], the Reviewer confirms whether the Coordinator has updated the status to Finalized — if not, it doesn't count toward the mandatory gate.
- After Iteration 5 ships, run the `claude-seo` plugin audit for a score.
- Iteration 7 depends on the X8 gate already existing: run a baseline, then compare post-change `score >= baseline - 10pp`.
