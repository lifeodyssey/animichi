...
- **Revision log:** see the bottom sections "Revision log (v2)" and "Revision log (v2.1)" — every change is mapped to the review finding it closes.
...
- `works.length >= 2`: let `maxP = max(priority)`, `top = works.filter(w => w.priority === maxP)`.
  ...
  **Tie = ambiguity.** The `priority` column is the tie-breaker; genuine ambiguity is a *tie at the top rank* among distinct works that actually share the normalized alias row. **Precondition to verify before relying on this (§7):** whether real ambiguous franchises actually share `alias_normalized` rows at equal priority — if they never do, the HIT path never clarifies and MISS-path exact-match (below) is the sole disambiguation source.
...
```
Let `subjects = fetchBangumiSubjects(query)`; `exact = subjects.filter(s => normalizeAlias(s.name) === normalizeAlias(query) || normalizeAlias(s.name_cn ?? "") === normalizeAlias(query))`.
- `subjects.length === 0` → `not_found`.
- `exact.length >= 2` → `needs_disambiguation` (candidates = `exact`, capped, enriched from the parsed subject fields).
- else (`exact.length <= 1`, incl. a soft relevance match) → `resolved(subjects[0])`. This is exactly today's `bestSubjectId` head-pick; it then flows to the existing tiered preview/ingest (`resolvePreview`/`missResult`, :129-151), returning `partial:true` (L1 preview) or a downstream `empty` (Bangumi resolved but Anitabi 404/empty). **Deliberately no "2+ subjects → clarify" rule** on the MISS path: Bangumi returns many relevance-ordered hits with no numeric score, so the only deterministic ambiguity signal we have is normalized-name exact-match count — using anything looser would explode the clarify partition.
...
- MISS, 2 normalized-name-exact subjects → `needs_disambiguation`. -> unit
- MISS, soft relevance head only → `resolved` (preserves head-pick). -> unit
...
> **Eval note:** the resolver's `outcome` partition IS the #1 eval gate (§7). Because today's `LIMIT 1` HIT path *never* clarifies, dropping it can flip cases **search → clarify** (a genuine tie now clarifies) as well as clarify → search (heuristic-only cases now search). §7's sign-off audit covers **both** directions.
...
| `response_builder.agent_result_to_response` | `response_builder.py:47-80` | `_UI_MAP.get(result.intent)` (server-owned); branch on output **type** (isinstance), not `output.intent`; build `data` from `result.session_state` registry, not `output.data`; add a `PartialResponseModel` branch (§4h) using `result.status`/`result.success` + `last_result_ref` projection |
| `_UI_MAP` | `response_builder.py:13-23` | drop BOTH dead keys — `answer_question` (:19, collapse to `general_qa`) AND `unclear` (:21) — and ADD `"partial": "GeneralAnswer"` (§4h/§5); keep all other values stable |
...
| chat pending-detection | `chat.py:49-92` (`_scan_parts_for_clarify`/`_detect_clarify_context`) | DELETE the body-scan; load `pending_clarification` from the persisted session via `x-session-id`; accept the choice via the mapped `selected_candidate_id`/`clarification_id` (selection contract below) |
...
| eval `_available_data_keys` | `evaluators.py:141-148` | stop reading `getattr(result.output, "data", ...)` (deleted); project legacy logical keys from `result.tool_state`/`session_state` (§7 compatibility projection) |
| eval stage→chains | `evaluators.py:67-76` | unchanged — kept green by the synthesized `clarify` step (§7) |
...
| analytics / span attrs | `public_api.py:210-235` (`runtime.intent`) | reads `response.intent` (same string values) |
...
|---|---|---|---|---|---|---|
| `resolve_anime → needs_disambiguation` | `ResolveAmbiguous` | `anime_ambiguity` | `candidate_ids=[≥2 bangumi_ids]`, `ordered_candidates=[…]`, `revision=++` | `selected_candidate_id ∈ candidate_ids` + `clarification_id == revision` | `search_bangumi(selected_candidate_id)` | clear pending, bump revision |
...
| `search_nearby → place_ambiguity` | `NearbyPlaceAmbiguous` | `place_ambiguity` | `candidate_ids=[≥2 gazetteer ids]` (from `geocode_staging`), `ordered_candidates=[…]`, `revision=++` | `selected_candidate_id ∈ candidate_ids` + `clarification_id == revision` | `search_nearby(location=<gazetteer label/id>)` | clear pending, bump revision |
...
- **No-candidate consume + CLEAR:** for the free-text reasons (`anime_not_found`/`place_too_broad`/`unknown_place`/`missing_location`) there is no `selected_candidate_id`; the next user turn is ordinary `text`. The server does NOT relatch — `pending_clarification` is set ONLY by a clarify-producing outcome (always with a fresh, monotonically bumped `revision`) and is cleared the moment the next turn yields any non-clarify terminal outcome (a `resolved→search`, `ok`, route, greet, qa), or replaced by a fresh-revision pending if the retry clarifies again. This is the explicit CLEAR transition (§4e) that replaces the sticky OR (`session_facade.py:165`).

**Clarify-SELECTION request contract (P1-4).** The current chat path detects a pending clarify by scanning history for the removed `clarify` tool call — that must be replaced by an explicit request field + server validation:
- `PublicAPIRequest` gains `selected_candidate_id: str | None` and **`clarification_id: int | None`** (`schemas.py:14-53`). **`clarification_id` is `int` to match `pending_clarification.revision: int` (§4e)** — a `str` field would make `str == int` always `False` and silently reject every valid selection (sonnet P3-a). *(If the transport can only carry a string, the server MUST coerce `int()` before comparison and reject a non-integer as a stale/invalid click.)* `selected_candidate_id` is the anime/place chosen from a clarify card (distinct from the existing `selected_point_ids`, which is route selection).
- The server validates `selected_candidate_id ∈ pending_clarification.candidate_ids` **and** `clarification_id == pending_clarification.revision` (a monotonically-increasing per-session token). A mismatch → the click is **stale** (the user clicked an old card) and is rejected with a typed "this choice expired, please try again" response — it does NOT re-run resolution on a stale id.
- On a valid selection the server **consumes** it (routes straight into `search_bangumi(selected_candidate_id)` / `search_nearby(location=<gazetteer id/label>)`) and **clears** `pending_clarification` + bumps `clarification_revision` (§4e). For `place_ambiguity` the selected gazetteer id/label is fed to `search_nearby(location=)` so a place choice reaches the tool by stable id, not re-typed prose (closes the B4 place-selection gap).

**Why best practice.** `ToolOutput(name=)` per stage is the idiomatic pydantic-ai multi-output pattern; but because the tool name is not on `output`, a *server-owned* stage map is the correct place to recover top-level intent — and deriving sub-intent from steps removes a whole class of model-lie failures. Validating `candidate_ids == authoritative state` by ordered equality *and* `reason` *and* cardinality is the trust-boundary discipline the codebase already applies to search/route, extended completely to clarify. The transition table makes the reject-when-no-pending guard *total* (no legit outcome can trip it). The revision token is the standard defense against stale optimistic-UI clicks.
...
- **Compact-output rule (lands atomically with §4c):** *"message = one short sentence. clarify carries candidate_ids and NOTHING else you transcribe. The ONLY identifiers you may ever echo are the candidate_ids handed to you by a tool outcome — never transcribe points, coordinates, other IDs, counts, titles, or route legs; the app fills all data from tool state."* (This is the explicit resolution of the "echo candidate_ids vs never-transcribe-IDs" contradiction, P2-1/#4: `candidate_ids` is the sole permitted ID echo.)
...
- **SSE / step model:** synthesize the "thinking" step at the runner level, or move to native `run(event_stream_handler=)` `FunctionToolCallEvent`. **Also synthesize the `clarify` step** when the output is `ClarifyResponseModel` (§7 — keeps the frozen eval chains green).
...
|---|---|---|
| `clarify` · `anime_ambiguity` | `Clarification` | cover-card picker over hydrated candidates; selecting one posts `selected_candidate_id` (its `bangumi_id`) + `clarification_id` (→ `search_bangumi`) |
| `clarify` · `place_ambiguity` | `Clarification` | place-choice list (gazetteer labels); selecting posts `selected_candidate_id` (→ `search_nearby(location=)`) |
...
Selection loop: a clarify card selection posts `selected_candidate_id` + `clarification_id` (the revision token). The server **validates against pending state**, rejects stale clicks, **consumes** the choice, and **clears** `SessionState.pending_clarification` (§4c/§4e) — no sticky re-clarify. Route selection continues to use `selected_point_ids`.
...
Deterministic `resolve` (dedup-by-work_id, tie rule, MISS exact-match, enrichment, `upstream_unavailable`) + `pointsByWorkId` + `fetchBangumiSubjects` (§4a). **MISS subject shape uses the shared parser's REAL fields — `images.{large,common,…}` for cover + `date`(fallback `air_date`) for year, NOT `image`/`air_date` (P2-6 / P3-c); `points_count` derived via `COUNT(points.id)` in the resolver query (enrich UPSERT does not own it).** Additive: the agent does not call them yet. -> integration (TS worker) + unit
...
- Request/response adapters: `PublicAPIRequest.selected_candidate_id` + **`clarification_id: int`** + server validation + selection-consume/clear (§4c). -> unit + api
- `_available_data_keys` compatibility projection (SessionState/tool_state → legacy logical keys) + pinned score snapshots (§7). -> unit + eval
- **Runner synthesizes the `clarify` step** when emitting `ClarifyResponseModel` (§7). -> unit + eval
...
### The frozen-baseline preservation strategy (P1-1, both reviews) — DEFAULT
`evaluators.py:67-76` hardcodes stage→chains: `"clarify": (("clarify",),)` and `"clarify_after_nearby": (("geocode","clarify"),)`. The `clarify` step is recorded today **by the clarify tool** (`tool_runtime.py:171-178`). §4b removes that tool → without mitigation `ToolCallRecall`/`RouteOrderCorrect`/`StepEfficiency` regress for ~163 frozen cases (72 single-clarify + 7 clarify_after_nearby in `agent_eval_v3.json`, 84 clarify cases in `runtime_journey_v1.json`). (greet/QA are SAFE — their chains include a `()` zero-step alternative; clarify has none.)

**DEFAULT strategy (mark as default; owner may override):** the runner **synthesizes a step named exactly `ToolName.CLARIFY.value`** when it emits `ClarifyResponseModel` (§4h). The synthesized trajectory for an anime clarify is `[resolve_anime, clarify]` — **byte-identical** to today's `[resolve_anime, clarify]` (StepEfficiency, F1, and route-order all match). For the nearby path, **confirm the internal `geocode` step still fires** (`ToolName.GEOCODE` exists, `models.py:25`; `search_nearby` records it) so `("geocode","clarify")` still matches with the synthetic clarify step. With this, `agent_eval_v3.json` **and** `apps/agent/agent/tests/eval/baselines/` stay **byte-identical** for all non-flipping clarify cases.

### `_available_data_keys` compatibility projection (P1-1 / sonnet P3-b) — ground-truth-corrected
**Ground truth (pinned):** NO frozen case in `agent_eval_v3.json` **or** `runtime_journey_v1.json` carries a `data_keys` metadata field (v3 metadata carries only `acceptable_stages` + `expect_nonempty`; journey carries only `expected_stage`; `grep '"data_keys"'` over `apps/agent/agent/tests/eval/` = 0 hits). `DataKeysPresent.evaluate` (`evaluators.py:189-193`) **short-circuits to `1.0` on an empty expected set** (`if not expected: return {"data_keys_present": 1.0}`) **before ever calling `_available_data_keys`**. Therefore removing the `output.data` branch (`evaluators.py:143-147`) is **provably score-neutral for every frozen case** — the function is never reached for them. (Codex P1-1 rated this 10/10 assuming the branch is on the hot path; against ground truth it is not — sonnet's P3-b "gated by the AC, verify during impl" is the correct severity. It is fixed here regardless, with the projection made explicit for the additive dataset + any future case.)

**Specified compatibility projection.** Rewrite `_available_data_keys(result)` to project legacy logical keys from the new state (so any case that DOES assert `data_keys` — notably the additive `agent_redesign_v1.json` — stays correct), retaining `result.tool_state.keys()` during migration:

| legacy `data_key` (was from `output.data`, `runtime_models.py`) | projected-from (new `SessionState`/`tool_state`) |
|---|---|
| `results` (search `SearchDataModel.results`) | `session_state.search_results` non-empty (any ref) |
| `route` (route `RouteDataModel.route`) | `session_state.routes` non-empty |
| `candidates` (clarify `ClarifyDataModel.candidates`) | `pending_clarification.ordered_candidates` non-empty |
| `options` (clarify `ClarifyDataModel.options`) | `pending_clarification.candidate_ids` non-empty |
| `question` (clarify `ClarifyDataModel.question`) | `pending_clarification is not None` |
| `status` (clarify/QA `…DataModel.status`) | clarify (`needs_clarification`) or QA (`info`) stage present |

- **`NonemptyResults` dependency (score-neutrality, pin it):** the 26 `expect_nonempty` cases read `ctx.output.tool_state.get("search_nearby")["row_count"]` (`evaluators.py:203-204`). `tool_state` is RETAINED on `AgentResult` during the atomic switch (§4c), so `search_nearby` MUST keep writing `row_count` into `tool_state["search_nearby"]` (or the projection must reconstruct it) — otherwise those 26 cases regress. This is an explicit score-neutrality dependency.

**Verification ACs:**
- Synthetic `clarify` step is appended on `ClarifyResponseModel` and only then. -> unit
- `search_nearby` still records a `geocode` StepRecord on the place-choice path. -> unit
- `_available_data_keys` projects `{results}` for a search result, `{route}` for a route, `{candidates,options,question,status}` for a candidate-clarify, `{question,status}` for a no-candidate-clarify — from `SessionState`, with no `output.data`. -> unit
- **PIN representative evaluator vectors (incl. `data_keys_present` + `nonempty_results`) for one search, one route, one candidate-clarify, one no-candidate-clarify case BEFORE the switch; re-run AFTER and assert byte-identical.** (Do not claim byte-identical from an implementer guess — pin the numbers first.) -> eval
- `agent_eval_v3.json` + `baselines/` unchanged; re-run confirms no metric drift on non-flipping cases. -> eval

### Versioned evaluator + a NEW additive dataset for new behaviors
- **Version the evaluator** (introduce an evaluator version tag) so a future migration is explicit; today's metric semantics are preserved by the synthetic step (above).
- **Introduce a NEW additive dataset** `apps/agent/agent/tests/eval/datasets/agent_redesign_v1.json` (name illustrative) for the genuinely-new behaviors (discriminated outcomes, `upstream_unavailable`, the new reasons `anime_not_found`/`place_too_broad`/`unknown_place`, split place-clarify, stale-ref, partial results, selection protocol). `agent_eval_v3.json` + its baselines stay byte-identical; the new dataset gets its own baselines and MAY carry `data_keys` (exercising the projection above). -> eval
...
### #1 GATE (output-stage partition) — BOTH flip directions, owner sign-off
`runtime_journey_v1.json` asserts `expected_stage` on the **OUTPUT stage**. The redesign is safe **iff** the catalog `outcome` reproduces the prior clarify/search partition for genuinely-ambiguous cases. Because dropping `LIMIT 1` (§4a) makes the HIT path able to clarify, the partition can shift in **both** directions:
...
- **search → clarify:** cases whose title actually hits a top-priority *tie* now legitimately clarify.

**Enumerate ALL clarify cases by authoritative reason** — build a checked-in table (`eval/clarify_case_audit.csv`) covering the 72 single-clarify + 7 clarify_after_nearby cases in `agent_eval_v3.json` **and** the 84 clarify cases in `runtime_journey_v1.json` (counts per the dual review), each row = `{case_id, dataset, current_expected_stage, authoritative_reason, data-ambiguous?, flip_direction, sign-off}`. Reasons are drawn from the §4c enum (6 reasons). **State the alias-seeding precondition** (do ambiguous franchises actually share equal-priority alias rows — verify against the alias table before relying on the tie rule). **Every `expected_stage` change (either direction) requires owner sign-off** and is enumerated before merge. -> eval

### RED LINE
`agent_eval_v3.json` **and** `apps/agent/agent/tests/eval/baselines/` stay **byte-identical**. No baseline reset without explicit owner sign-off. The new `agent_redesign_v1.json` and its baselines are additive.
...
- Selection protocol: `selected_candidate_id` + `clarification_id` (int) validation + stale-click rejection. -> api + unit
...
- Catalog `resolve`/`pointsByWorkId` outcome partition + MISS subject field-name parse. -> integration (TS worker)
...
- **Frozen baselines:** `agent_eval_v3.json` + `baselines/` byte-identical, preserved by the synthetic `clarify` step + the `_available_data_keys` projection; new behaviors go to the additive dataset. Representative evaluator vectors PINNED before/after.
...
1. **MiMo thinking posture (eval-relevant).** MiMo (the default `openai_compat_base_url`) is a reasoning model with no thinking-control param; if thinking defaults ON it inflates request count. **Recommended default: verify MiMo's thinking posture and, if ON, disable/steer it** (via model profile, §4g); this changes eval numbers, so re-baseline the *new* additive dataset only (frozen baselines stay via the synthetic step).
...
- **§4c server-owned `RuntimeStage` map on `AgentResult`** + the atomic consumer-migration table (AgentResult.intent, response_builder, chat merge + pending-detection, persistence, analytics, eval, output models) + the clarify-**selection** request contract (`selected_candidate_id` + `clarification_id` + server validation + stale-click rejection). Closes RECONCILED 4, Codex P1-4.
- **§7 eval preservation** = synthetic `clarify`-named step (frozen baselines byte-identical) + geocode-step confirmation + versioned evaluator + additive `agent_redesign_v1.json` + direct thrash red-line gates (request/tool-call/repeat/p95) + both-direction sign-off audit enumerating all 72+7 v3 and 84 runtime_journey clarify cases by reason + alias-seeding precondition. Closes RECONCILED 5, Codex P1-6, sonnet P1-1/P1-3.
