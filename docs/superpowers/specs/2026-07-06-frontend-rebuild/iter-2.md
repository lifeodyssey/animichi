...
- Multi-turn: after completing a Walk on some route and returning to `/routes`, that card's completed status reflects correctly without a manual refresh -> integration
...
- Data integrity: after migration, a random 10% sample of rows gets a field-level content comparison (not just a row count) to confirm the `state`/`metadata` JSONB content wasn't corrupted -> integration
...
- Accuracy (hard AC, eval-first): across 20 sampled popular-anime routes, the estimated duration compared against Jorudan's real-world figures has a P80 error **≤ ±10 minutes** -> eval
...
- Empty: when two stops don't require a railway transfer (close enough that walking is preferable), no `TransitLeg` renders — the existing walking-leg display is preserved -> unit
