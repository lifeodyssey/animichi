...
- **SD-2 (final, not under discussion)**: the access path for user-domain data (saved/listed routes) is **API-first** — a newly built `workers/users` TS service exposing oRPC routes at `/v1/users/*`, with data landing in **Neon**, and contracts placed in `packages/contract`; `apps/web`'s **Neon Auth SDK (Better Auth)** is used **only** for auth (to obtain the JWT) and does **not** query any data table directly, nor does it rely on RLS (SD-31 — the auth backend is Neon Auth, was `supabase-js`). The earlier rev 1-4 draft's provisional framing of "currently written against a tentative direct-RLS design" is now void — this file has been written against the SD-2 final decision from the outset.
...
- Auth: requests carry the **Neon Auth**-issued JWT as a bearer token; `workers/users` validates it **against the Neon Auth JWKS** (endpoint injected via `NEON_AUTH_JWKS_URL`, not hardcoded) and takes `sub` to resolve the Neon-side `user_id`; `apps/web`'s **Neon Auth SDK** is used only to obtain this JWT (via its session/token API) — it does not query any data table. (SD-31; was a Supabase-issued JWT + `supabase-js`.)
...
- **Does not use** RLS as the access path (RLS is now Neon-native in the `neon_auth`/data schemas per SD-31 — kept as defense-in-depth, not the access path; SD-2 explicitly overturns the earlier rev 1-4 draft's provisional approach); **no** data endpoints are added to `apps/agent`.
...
**Changed files**: `scripts/build-station-topology.py` (new, ekidata.jp + N02 ETL pipeline), `apps/agent/agent/infrastructure/transit/topologyGraph.py` (new, graph construction + Dijkstra), `apps/agent/agent/infrastructure/transit/stationIndex.py` (new, spot → nearest-station index), `apps/agent/agent/agents/models.py` (extended, adds `mode: "transit"` support to the existing `TransitLeg` output type — NOT `route_optimizer.py`, which is retired in iteration 1 per S1.7/SD-28; route-ordering logic lives in `workers/catalog/src/lib/route.ts`), `apps/agent/agent/tests/eval/transit_estimate_accuracy.py` (new, 20-route comparison against Jorudan), `apps/web/src/components/route-detail/TransitLegRow.tsx` (new, renders the summary + deep-link button + N02 attribution footer).
