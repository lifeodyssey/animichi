...
**Authority annotation (backfilled from main spec inputs §10 Step2/Step5; corrected per SD-30 review, Codex P1 — the main spec is authoritative and still lists exactly two [proposal, pending confirmation] items)**: This iteration's content is **almost entirely finalized, with exactly two items still [proposal, pending confirmation] per the main spec §8.8 / §④ note**: (1) **P6** (the SSE disconnect semantics — no-resume-on-disconnect + `GET messages` fallback) and (2) the **message-length cap**. Everything else below is finalized:
- **SD-9 (protocol)**: The AI SDK UI message stream via `VercelAIAdapter` is finalized (replacing the original three-event-SSE proposal, and also replacing the standalone `turn_id`/`seq` field design that used to hang off the three-event schema — those field-level designs are voided along with the three-event protocol). **Note**: the *protocol choice* is finalized, but **P6's disconnect-semantics detail (no resume on disconnect + fetch final state via `GET messages`) remains [proposal, pending confirmation]** — the Coordinator must walk the user through it before it becomes a merge-blocking gate; the story ACs below carry it as the default, tagged pending.
...
- **Message length ceiling**: **[proposal, pending confirmation]** per the main spec §8.8 — a complete plan exists (a Guardrails addendum), but the user-discussion step was skipped, so it stays pending until the Coordinator confirms; the S1.12 AC carries it as the default with the value marked pending.
...
- **Disconnect-recovery semantics ([proposal, pending confirmation, P6] — per the main spec §8.8, this remains pending user sign-off; the AC below is the default, not a merge-blocking gate until the Coordinator confirms)**: if a disconnect occurs mid-stream, no attempt is made to resume the original stream (mid-stream resumption is unsupported; reuse the AI SDK's resume capability directly if it has one, otherwise do not build custom resumption infrastructure); the UI, for any path other than the `finish` event arriving, transitions into S1.6's D4 exception state, and the client instead calls `GET /v1/conversations/{id}/messages` to fetch the session's final state -> browser [proposal, pending confirmation]
...
- **Disconnect-recovery semantics ([proposal, pending confirmation, P6] — per the main spec §8.8, this remains pending user sign-off; the AC below is the default, not a merge-blocking gate until the Coordinator confirms)**: D4's (stream interruption) recovery semantics are "no support for resuming a broken stream; the client instead calls `GET /v1/conversations/{id}/messages` to fetch the current session's final state," rather than attempting to resume the original AI SDK stream -> browser [proposal, pending confirmation]
...
- **Message-length ceiling ([proposal, pending confirmation] per the main spec §8.8 — the AC is kept as the default but is not a merge-blocking gate until the Coordinator confirms)**: user input beyond a configured length ceiling (**4000 characters — initial value via env `MESSAGE_MAX_CHARS`; pending final confirmation**) or of a non-text type is rejected, showing "メッセージが長すぎます" (or equivalent) copy, and is never sent to the agent -> unit [proposal, pending confirmation]
