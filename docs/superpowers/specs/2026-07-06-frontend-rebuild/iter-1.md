...
Suggested dependency order: S1.1 → S1.2 → {S1.3, S1.4 (depends on S0.4), S1.5 (depends on S0.4)} → S1.6 → S1.7 → S1.8 → {S1.9, S1.10} (parallel, both depend on S1.8) → S1.11 → S1.12 (can run in parallel with S1.8; **no data-interface dependency on S1.8** — the original P3-middleware dependency is voided now that P3 was cut under SD-18, and S1.8's circuit breaker sources its data from the `daily_usage` table instead; see S1.12's dependencies note) → S1.13 (does not block any other story and can run in parallel at any point; depends on S0.1's CI-gating infrastructure).
...
**Authority annotation (backfilled from main spec inputs §10 Step2/Step5; corrected per SD-30 review, Codex P1; updated by the P3 patch, 2026-07-07 — the main spec is authoritative and now lists ZERO pending-confirmation items)**: This iteration's content is **now fully finalized**. The last two items that were previously pending — (1) **P6** (the SSE disconnect semantics — no-resume-on-disconnect + `GET messages` fallback) and (2) the **message-length cap** — were confirmed by the user in the "spec 收口三件" ruling and are both now **Finalized (finalized 2026-07-07)**, exactly as written into the story ACs. Everything below is finalized:
- **SD-9 (protocol)**: The AI SDK UI message stream via `VercelAIAdapter` is finalized (replacing the original three-event-SSE proposal, and also replacing the standalone `turn_id`/`seq` field design that used to hang off the three-event schema — those field-level designs are voided along with the three-event protocol). **Note**: the *protocol choice* is finalized, and **P6's disconnect-semantics detail (no resume on disconnect + fetch final state via `GET messages`, D4 exception card with retry) is now Finalized (finalized 2026-07-07)** — it may serve directly as a merge-blocking gate; the story ACs below carry it as the confirmed default.
- **S1.11's P8 (SSRF egress guard)**: finalized under **SD-20 (finalized 2026-07-06)** as a strict post-resolution IP check with no domain allowlist; no longer pending confirmation.
...
- **Message length ceiling**: **Finalized (finalized 2026-07-07, per the P3 patch)** — env `MESSAGE_MAX_CHARS`, initial value 4000 (initial value; executor may tune with evidence); the S1.12 AC carries it as the confirmed default.
...
- **SD-19 (backfilled)**: The full injection-defense suite (the P0 architectural invariant + web_search/tool-return delimiting, P1 source tiering, P2 Prompt Guard side-channel scoring, the hand-written eval G-1 cases), folded into S1.12, replacing the original `pending-confirmation` label.
...
- **SD-28 layer 0 (backfilled) + the dual-route unification (P3 patch)**: the walking-time estimate now applies a haversine × 1.3 detour coefficient, landing on the **TS catalog side** (`workers/catalog/src/lib/route.ts`, not the Python `route_optimizer.py`), folded into S1.5; and the `selected_route` bypass's route ordering switches from Python `route_optimizer.py` to `deps.catalog` (same surgery as the SD-3① fix), retiring `route_optimizer.py` (315 ln) — folded into S1.7. The greedy-NN + union-find algorithm is unchanged. Self-hosted OSRM/Valhalla (SD-28 layer 1) moves to Iteration 3 alongside Walk and is out of scope here.
...
- **Hard performance gate (SD-4)**: warm p95 first-token latency ≤3s (initial value; executor may tune with evidence), measured by a **defined probe — n=20 sequential `/v1/chat` requests after 1 warm-up request (discarded), measured server-side as request-received → first-SSE-byte-emitted, from a single region against a pre-warmed container; p95 is the 19th-of-20 ordered value**; the story cannot be merged if this isn't met -> api
...
- Vision recognition is a **standalone vision call (per SD-26 D1 — phase 1 stays a standalone vision call, not folded into the main tool loop)**, routed by the **D4 vision-supply decision tree** (see the dedicated backend enabler below): the baseline is an independent vision call against platform Gemini, and a BYOK key is used **only when it has been capability-probed as `vision_capable`** — it is **not** assumed that all three BYOK provider families support vision. No dedicated recognition service is added; the recognition result is formatted as a candidate-title list (**at the series level**, echoing the 04-27 series-aware `resolve` design) and handed to `resolve_anime`, which follows the existing DB-first → API-fallback path; no new tool is created.
...
- **Photo-search happy path (backfilled from SD-26 phase 1, replacing the original "degrade to apology" AC)**: uploading the named fixture **`apps/agent/src/animichi/tests/fixtures/vision/yourname_screenshot.jpg` (expected recognition: 君の名は。 / bangumi_id 160209)**, vision recognizes the title and triggers series-aware `resolve_anime`, ultimately rendering that title's pilgrimage map (sharing its render path with the text-search C3a/C3b) -> integration
- Empty: uploading the named fixture **`apps/agent/src/animichi/tests/fixtures/vision/unknown_landscape.jpg` (a non-anime landscape photo, expected: no candidate title recognized)**, it degrades to a C2 clarifying follow-up ("which title is this?" + a manual-entry chip), reusing the existing clarify branch without introducing a new failure-mode mechanism -> browser
...
**Backend enabler (backfilled from SD-28 layer-0; per the P3-patch dual-route unification, this lands on the TS catalog side — the X12 "route logic belongs to the catalog" direction)**: the walking-time estimate applies a **haversine × 1.3 detour coefficient** on top of the existing haversine÷80m/min estimate, and this coefficient lands in `workers/catalog/src/lib/route.ts` (the TS catalog route logic) — **not** the Python `route_optimizer.py`. Route ordering/estimation is being unified onto the catalog: S1.7's dual-route unification switches the `selected_route` bypass to `deps.catalog` and retires `route_optimizer.py` altogether, so the ×1.3 change is authored once, on the catalog side, rather than in a Python file that is about to be deleted. Real urban street-network detour coefficients average ~1.3 (initial value; executor may tune with evidence); after the correction, the residual error is already smaller than the dwell-time estimation error, which is good enough for the planning stage. Self-hosted OSRM/Valhalla routing (SD-28 layer 1) moves to Iteration 3 alongside Walk and is **not** in scope for this iteration — its real value there is track-polyline rendering + barrier detection (e.g., a straight line of 100m across a river that's actually a 1km walk), not timing precision, so adding it now would be premature engineering.
...
- **The walking-time estimate applies the ×1.3 detour coefficient (initial value; executor may tune with evidence) on top of the haversine÷80m/min baseline, implemented in `workers/catalog/src/lib/route.ts` (the TS catalog side, per the P3-patch dual-route unification), with the regression test living in the catalog test suite** -> unit
...
**Files changed**: `apps/web/src/components/chat/TimedItinerary.tsx`, `apps/web/src/components/map/RouteTrailMap.tsx`, `apps/web/src/components/chat/registry.ts`, `workers/catalog/src/lib/route.ts` (the ×1.3 detour coefficient, TS catalog side per the P3-patch dual-route unification), `workers/catalog/test/route.worker.test.ts` (coefficient regression test in the catalog suite).
...
- **Disconnect-recovery semantics (P6 — Finalized 2026-07-07 per the P3 patch; a merge-blocking gate)**: D4's (stream interruption) recovery semantics are "no support for resuming a broken stream; the client instead calls `GET /v1/conversations/{id}/messages` to fetch the current session's final state," rather than attempting to resume the original AI SDK stream -> browser
...
**Dual-route unification (backfilled from the P3 patch / the SD-28 supplement — the same surgery as the SD-3① fix, X12 "route logic belongs to the catalog" direction)**: beyond the data-source fix above, the `selected_route` bypass's route-*ordering* logic also switches from the Python `route_optimizer.py` to **`deps.catalog`** (the TS catalog route logic in `workers/catalog/src/lib/route.ts`, where the ×1.3 detour coefficient added in S1.5 also lives) — reached through the same `CatalogClient` route endpoint the chat-path uses. The greedy nearest-neighbor + union-find 50m-clustering algorithm itself is **unchanged** (the wheels round-2 ruling kept it; the ST_ClusterDBSCAN alternative was withdrawn) — only its owner moves from Python to TS. Once the bypass reads its points from Neon and orders them through `deps.catalog`, the Python `route_optimizer.py` (315 lines) has **no remaining production caller and is retired (deleted) in this story**. This collapses the two route paths — the chat-path `plan_route` and the `selected_point_ids` bypass — onto a single catalog implementation, so both produce identical ordering for the same selection.
...
- **Dual-route ordering parity (the P3-patch dual-route unification)**: for a fixed `selected_point_ids` selection, the bypass's route ordering now comes from `deps.catalog` (the TS catalog route logic) and matches the ordering the chat-path `plan_route` produces for the same selection — an integration test asserts an identical route-stop order (ordered point IDs — not S2.10's railway-station vocabulary) across both paths for one fixed selection -> integration
- **`route_optimizer.py` retirement (the P3-patch dual-route unification)**: after the switch, the Python `route_optimizer.py` has no remaining production caller — a unit/static check asserts no non-test import of `route_optimizer` remains anywhere in the `apps/agent` package, and the module is deleted -> unit
- **P5 login-wall trigger timing (backfilled, Fable P1-5)**: the magic-link modal opens **only** when 「保存する」 is tapped — a browser test asserts the modal is not present in the DOM / not opened at any point until that tap -> browser
- **P5 no early interruption (backfilled, Fable P1-5)**: an anonymous user completing the full planning flow (search → clarify → route generation → follow-up refinement → checkbox reorder) is never interrupted by a login prompt at any step before the 「保存する」 tap — an integration test walks the flow and asserts no auth challenge fires earlier -> integration

**Login-wall trigger** (P5, and nowhere else): tapping 「保存する」 → opens the magic-link modal (reusing S0.6's `LoginModal`); the account-claiming logic for user-domain data like saved/favorited routes after a successful login **is still implemented in S2.8** — this story is only responsible for the trigger timing and the login UI, not for rebuilding the login component. **S1.7 ↔ S2.8 boundary (C4 ruled complementary — no longer pending confirmation)**: SD-15② is finalized as "anonymous-to-logged-in session-ownership migration (device token → user_id) lands in Iteration 1," and this story accordingly owns the session-memory/fact-ledger-level ownership migration above; S2.8 (Iteration 2) separately owns the account-claiming of user-domain data like "saved routes." The two operate on **different data layers** (session/memory vs. route/favorites) and are **complementary, not overlapping** — this is a settled cross-reference, kept here for traceability, not an open scheduling question.
...
**Files changed**: `apps/web/src/components/chat/LivingDocument/*`, `apps/web/src/components/chat/SelectionTray.tsx`, `apps/web/src/lib/chat/selectedPointsBypass.ts`, `apps/agent/agent/agents/selected_route.py` (switched to `CatalogClient` for points + route ordering delegated to `deps.catalog`), `apps/agent/agent/agents/route_optimizer.py` (**deleted — retired once the bypass switches to `deps.catalog`**), `workers/catalog/src/lib/route.ts` (the catalog-side route logic that now owns ordering for both paths), `apps/agent/agent/tests/unit/test_selected_route.py` (regression snapshot + the no-remaining-`route_optimizer`-import assertion), `apps/agent/agent/tests/integration/test_dual_route_parity.py` (new, the ordering-parity test across chat-path and bypass), `apps/agent/agent/domain/fact_ledger.py` (new, the SD-15 typed fact ledger), `apps/agent/agent/agents/session_ownership.py` (new, anonymous-to-logged-in ownership migration), `apps/agent/agent/agents/history_compaction.py` (verbatim-snippet retention logic).
...
### S1.11 BYOK (bring your own LLM key, three provider families, finalized under SD-11) + the SSRF egress guard (backfilled from SD-20, now finalized, replacing the original `pending-confirmation` P8 label)
...
- **Quota-boundary: BYOK bypasses the dollar budget (backfilled from SD-20, Fable P1-4)**: a request carrying a valid BYOK credential is **not** counted against the X4 global daily dollar-budget circuit breaker and is allowed through even when the platform anonymous budget is exhausted (a unit test asserts the breaker is skipped for BYOK requests) -> unit
- **Quota-boundary: BYOK is still fully guarded (backfilled from SD-20, Fable P1-4)**: a BYOK request still passes through injection detection (S1.12) + `output_validator` + content guardrails + edge rate limiting / anomalous-frequency detection — an integration test asserts a BYOK request with an injection payload is still flagged and a BYOK request over the rate limit is still throttled, so BYOK can't be used as a backdoor to bypass Turnstile / rate limits and hit downstream APIs -> integration

**Quota boundary (backfilled from SD-20)**: BYOK is exempt from the X4 global daily cost budget, but **not** exempt from injection defense (S1.12) / `output_validator` / content guardrails / anomalous-frequency detection — this prevents someone from using BYOK as a backdoor to bypass Turnstile and hit downstream APIs. (The two ACs above pin this down.)
...
### S1.12 The full agent injection-defense suite (backfilled from SD-19, now finalized, replacing the original `pending-confirmation` P2/P3 labels) + a message-length ceiling + guardrails tech-debt closeout

**User story**: As a site operator, I want the agent to explicitly flag external search/tool-result content as untrusted when quoting it (guarding against prompt injection), to have an architectural-level invariant written into the system prompt ensuring that "anything arriving via a tool/MCP/A2A can never be promoted to an instruction," to apply basic source tiering, to have a non-blocking side-channel detection net as a backstop, and to cap the length of user input to prevent abuse or anomalous input from overwhelming the system. **The injection-defense portion of this story is now finalized under SD-19 (2026-07-06) and is no longer `pending-confirmation`; the original draft's P2 (delimiting) / P3 (timing middleware) numbering is consolidated/adjusted here, see below. The message-length ceiling is now Finalized (finalized 2026-07-07 per the P3 patch — env `MESSAGE_MAX_CHARS`, initial value 4000; see its AC below).**
...
- **Message-length ceiling (Finalized 2026-07-07 per the P3 patch; a merge-blocking gate)**: user input beyond a configured length ceiling (**4000 characters via env `MESSAGE_MAX_CHARS`; initial value, executor may tune with evidence**) or of a non-text type is rejected, showing "メッセージが長すぎます" (or equivalent) copy, and is never sent to the agent -> unit
