...
**Core AC**:
- Happy path: given a query image + a known series scope, the embedding coarse screen returns the top 20-30 candidates across every spot in that series (including spots belonging to other anime in the same series union), within an acceptable response time (the exact threshold is set by the executor based on measured data, recorded as an AC assertion rather than guessed in advance) -> integration
...
- Performance threshold (a verification assertion for the "no ANN index" decision): at series-merged scale (up to ~1200 points), pgvector brute-force scan query latency stays locked within a performance-regression-test threshold; exceeding it logs an alert and triggers a DD-12 (ANN indexing) re-evaluation, rather than silently degrading -> integration
...
- **Evaluation-set scale (hard AC, backfilled from SD-30 L3)**: the offline A/B evaluation corpus targets **100-150 labeled pairs**, structured as 10 anime titles × 10 shot-angles (機位) × two query modalities (an anime-frame query and a real-scene-photo query) -> eval
...
**Changed files**: `apps/agent/agent/infrastructure/vision_search/embeddingIndex.py` (new, coarse screen), `apps/agent/agent/infrastructure/vision_search/visionRerank.py` (new, rerank; also implements the 2-batch cap, canary check, and low-confidence routing), `apps/agent/agent/agents/tools/match_scene.py` (new, the `@agent.tool` registration exposing the unified `match_scene(image, scope: {bangumi_ids} | {geo, radius})` signature, internally orchestrating embeddingIndex → visionRerank), `apps/agent/agent/tests/eval/vision_search_ab.py` (new, offline A/B evaluation, including the 100-150-pair labeled corpus), Neon `spot_embeddings` table migration (a `halfvec(1536)` column, via the SD-1 tool chain).
...
- **real2real fast-path unlock**: once a given spot's accumulated check-in photo count crosses a preset threshold, subsequent recognition requests for that spot are flagged eligible for the real2real fast path (distinct from the default anime2real cross-domain path); spots below the threshold keep using S4.8's default path -> integration
