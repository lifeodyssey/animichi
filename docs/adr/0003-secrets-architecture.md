...
- New `infra/` module: a `neon` Pulumi stack declaring `neon.Role` ×4 (catalog_svc, agent_svc, users_svc, jobs_svc) with passwords; exports connection strings as stack outputs (secret-marked). **(Superseded for `catalog_svc`/`agent_svc`/`users_svc` by the 2026-09-23 amendment: one `neon.Role` remains, `migrator`; the runtime roles are SQL-created and carry Pulumi-generated `random.RandomPassword`s. `jobs_svc` was never declared by the stack: the migrator's SQL step ensures it among the five service roles — created when absent, held NOLOGIN with its memberships revoked — and only the three runtime roles get passwords from that step.)**
...
- Atlas migrations keep `CREATE ROLE IF NOT EXISTS` (idempotent) and all GRANTs; role LOGIN state is driven by the Pulumi-managed role (Neon sets password → role is login-capable). **(Superseded for role creation by the 2026-09-23 amendment: the chain issues no `CREATE ROLE` — role DDL left the chain with #1636, and the migrator's SQL step now ensures the five service roles and drives their LOGIN state and passwords, issuing no GRANTs itself; what remains of the chain's role work here is the precheck that all five service roles exist. The chain's GRANTs all stand: `GRANT USAGE ON SCHEMA public` to the five data-plane roles, its per-table grants — `DELETE, INSERT, SELECT, UPDATE` to `catalog_svc`, `users_svc` and `agent_svc` (`daily_usage` without `DELETE`), `INSERT, SELECT` to `agent_svc` on `pi_records`, `SELECT` to `readonly`, `SELECT` to `jobs_svc` on `saved_routes`, `DELETE, SELECT` to `jobs_svc` on `sessions` and `anon_daily_message_count` — and `GRANT SELECT, USAGE` on the catalog history sequence to `catalog_svc`.)**
