...
green_case("fixture validates") do
  SecurityCheckRunsCanary.assert!(copy_fixture, repo: REPO, expected_sha: SHA,
                                  required_contexts: ["CI / verify", "Review Gate"])
end

red_case("old required Security child is rejected", "exactly CI / verify and Review Gate") do
  contexts = ["CI / verify", "Review Gate", "Security / semgrep"]
  SecurityCheckRunsCanary.assert!(copy_fixture, repo: REPO, expected_sha: SHA,
                                  required_contexts: contexts)
end

red_case("duplicate CI check runs are rejected", "exactly one CI / verify check run") do
...
green_case("live PR mode resolves the real head and ruleset") do
  checks_path = "/repos/#{REPO}/commits/#{SHA}/check-runs?per_page=100"
  client = live_client(["CI / verify", "Review Gate"])
...
green_case("live mode follows later check-runs and ruleset pages") do
  ...
  client = live_client(["CI / verify", "Review Gate"], check_pages: [filler_runs, copy_fixture.fetch("check_runs")],
...
red_case("live mode rejects empty check-runs results", "exactly one CI / verify check run") do
  client = live_client(["CI / verify", "Review Gate"], check_pages: [[]])
  SecurityCheckRunsCanary.run_live!(REPO, SHA, client: client)
end

red_case("live mode rejects empty ruleset results", "exactly CI / verify and Review Gate") do
  client = live_client(["CI / verify", "Review Gate"], ruleset_pages: [[]])
  SecurityCheckRunsCanary.run_live!(REPO, SHA, client: client)
end

red_case("live ruleset rejects an old Security child", "exactly CI / verify and Review Gate") do
  contexts = ["CI / verify", "Review Gate", "Security / semgrep"]
