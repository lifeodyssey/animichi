...
# concurrency must cancel PR-class runs (pull_request AND pull_request_target,
# each judged in its own event world) / never cancel push runs, and merge_group
# must cover every workflow producing a required context — with every
# REQUIRED_CONTEXTS owner workflow present in the scanned dir. Also regression-
...
Dir.mktmpdir("b7-red-missing-owner") do |dir|
  ...
  abort "FAIL: missing-owner expected 'ci.yml:top-level:missing required-context owner workflow' in output:\n#{out}" unless out.include?("ci.yml:top-level:missing required-context owner workflow")
end
