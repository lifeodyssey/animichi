...
import { guardTurnstile, verifySiteverify } from "../src/protect/turnstile.ts";
import { createTurnstileGate } from "../src/protect/turnstile.ts";
import { ENV, ID, request, unreachable, htmlGateway, withErrorLog } from "./doubles/turnstile-doubles.ts";
...
void test("an unreachable siteverify fails closed rather than 500ing the turn", async () => {
  const { result, records } = await withErrorLog(() =>
    verifySiteverify("t1", "", ENV.TURNSTILE_SECRET, unreachable),
  );
  assert.deepEqual(result, { ok: false, errorCodes: ["siteverify-unavailable"] });
...
void test("an outage returns a retryable challenge instead of throwing", async () => {
...
void test("the outage verdict is never cached — verification resumes at once", async () => {
  let attempts = 0;
