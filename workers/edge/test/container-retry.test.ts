...
/** The edge's own verdict for a caller it verified: the unmanaged
 * `GET /v1/conversations` read reaches `forwardV1` only after this. */
...
// `/v1` forward — chat, conversations, feedback, anything through
...
// forward here is the unmanaged `GET /v1/conversations` read: a route with no
// limiter cell, so the retry is the only thing between the request and the
// binding.
...
void test("/v1 retries a not-running 500 with 400/800ms backoff, then forwards the eventual success", async () => {
  ...
  const res = await app.request("/v1/conversations", {}, env, stubCtx);
