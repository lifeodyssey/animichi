...
// request never reached the database — and the container recorder is here to
// prove the opposite direction is not silently restored either.
...
function containerRecorder(forwarded: Request[]) {
  return { idFromName: () => "id", get: () => ({
    fetch: (request: Request) => { forwarded.push(request); return Promise.resolve(new Response("container")); },
  }) };
}

void test("the verified identity reaches the tier and the list never crosses into the container", async () => {
  ...
  const response = await app.request(PATH, { headers: { Authorization: "Bearer jwt" } },
    { ...env, CONTAINER: containerRecorder(forwarded) }, execution);
  ...
  assert.deepEqual(forwarded, [], "the conversation index is this Worker's now");
});

void test("an unauthenticated visitor gets a 401 without Turnstile, the tier or the container", async () => {
  ...
  const response = await app.request(PATH, {}, { ...visitorEnv, CONTAINER: containerRecorder(forwarded) }, execution);

  assert.equal(response.status, 401, "one account's index is never a visitor's");
  assert.deepEqual([turnstile, calls, forwarded], [[], [], []], "no anonymous pipeline, no tier, no container");
});
