...
import { DENIED_EGRESS_HOSTS } from "../src/container/container-env.ts";
...
const VENDORED_CONTAINER_JS_PATH =
  new URL("../../../node_modules/@cloudflare/containers/dist/lib/container.js", import.meta.url).pathname;
...
function extractRealMatcher(): { matchesHostList: (hostname: string, patterns: string[]) => boolean } {
  ...
  const { start, end } = matcherBounds(source);
  const globHelpersSource = source.slice(start, end);
  return evaluateMatcherFactory(globHelpersSource);
}

function matcherBounds(source: string): { start: number; end: number } {
...
function evaluateMatcherFactory(globHelpersSource: string) {
