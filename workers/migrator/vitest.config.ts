...
/** Migrator worker HTTP-seam tests (plain vitest; bounded apply/JWKS injected). */
export default defineConfig({
  ...
  test: {
    ...
    coverage: {
      provider: "istanbul",
      include: ["src/**/*.ts"],
      // apply-lock.ts is workerd Durable Object glue; ledger.ts is the Neon
      // HTTP adapter (live Postgres); bundled-chain.ts is compile-time SQL.
      exclude: ["src/ledger.ts", "src/apply-lock.ts", "src/bundled-chain.ts"],
      reporter: ["text", "lcov"],
