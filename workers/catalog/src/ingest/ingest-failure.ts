/**
 * The ingest failure taxonomy (#1784, extended by #1792): which kind of
 * failure a parked `ingest_jobs` row records, and what the person reading it
 * should do.
 *
 * Five kinds, one per distinct response:
 ...
 *   - `egress_refused`   — OUR anitabi egress service refused (its rate ceiling,
 *                          its auth, or it is misconfigured). This is us, not
 *                          the upstream — do NOT contact the upstream about it.
 *                          The ceiling clears within the hour, so the park is
 *                          short; auth/configuration need a fix on our side.
...
const OPERATOR_ACTION: Record<IngestErrorCode, string> = {
  ...
  egress_refused:
    "our own anitabi egress service refused; do NOT contact the upstream — a ceiling refusal clears "
    + "within the hour, an auth/configuration refusal needs the key or deploy fixed on our side",
...
export function classifyIngestFailure(err: unknown): IngestFailure {
  ...
  if (err instanceof EgressRefusedError) {
    return { code: IngestErrorCode.EgressRefused, cause: err.message, upstream: null };
  }
