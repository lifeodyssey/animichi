 ...
 * clean database, and provide its DSN to every integration test file. Any failure in
 ...
 * stays this arm's own is the database name and `SPIKE_SETUP_BUDGET`: one
 * container serves the whole suite, so it probes 30 × 1 s.
 */
const CLEAN_DATABASE = "catalog_integration";

export default async function setup(project: TestProject): Promise<() => Promise<void>> {
  const plane = await startTestPostgres({ database: CLEAN_DATABASE, budget: SPIKE_SETUP_BUDGET });
  project.provide("integrationDatabase", { enabled: true, dsn: plane.dsn });
  return () => plane.stop();
}
