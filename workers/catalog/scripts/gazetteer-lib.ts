...
import { compareText, sorted } from "./gazetteer-render";
...
export type CityNames = Record<string, { ja?: string; zh?: string }>;
export interface BuildInput { stations: unknown; cities: string; cityNames: CityNames; prefectures?: readonly Prefecture[] }
...
function addCapitalAlias(result: Gazetteer, pref: Prefecture): void {
  ...
  if (!capital) {
    warnMissingCapital(pref);
    return;
  }
  addCapitalAliases(result.aliases, capital, pref);
}

function addCapitalAliases(rows: AliasRow[], capital: LocationRow, pref: Prefecture): void {
  const bare = pref.capital.replace(/市$/u, "");
  addAlias(rows, capital.id, bare, "ja", priority[capital.kind]);
  addAlias(rows, capital.id, `${bare}市`, "ja", priority[capital.kind]);
  addBarePrefectureAliases(rows, capital, pref);
}

function warnMissingCapital(pref: Prefecture): void {
