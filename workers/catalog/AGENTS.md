...
- `*.integration.test.ts` runs in the Node pool via `vitest.integration.config.ts` for filesystem,
  ...
  **unless the check must never be skippable**. The suite is **hermetic and fail-loudly** (card
  ...
  container and applies the committed `migrations/neon` Atlas chain to a database of its own on it
  — the plane's own database is migrated by the Prisma chain now (#1625), and this query layer still
  reads the Atlas-era shape, so the suite keeps a pre-Prisma database of its own until #1628–#1631
  land (`integration-db-global.ts` carries the same note). Any setup failure throws — there is no
