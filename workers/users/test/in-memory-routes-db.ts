...
function routeRowBase(values: unknown[], status: RouteStatus): Omit<FakeRouteRow, "saved_at" | "updated_at" | "first_query"> {
  return {
    id: NEW_ID,
    session_id: null,
...
function executeText(query: { text: string; values: unknown[] }, rows: FakeRouteRow[]): unknown[] {
  const { text, values } = query;
  if (text.includes("select user_id")) return selectUserId(rows, values);
  if (text.includes("insert into routes")) return insertRoute(rows, values);
  if (text.includes("delete from routes")) return deleteRows(rows, values);
  ...
  if (text.includes("update routes")) return updateRoute(rows, values);
  return routeRows(rows, values);
}

function selectUserId(rows: FakeRouteRow[], values: unknown[]): unknown[] {
  const row = rows.find((item) => item.id === values[0]);
  return row ? [{ user_id: row.user_id }] : [];
}

function insertRoute(rows: FakeRouteRow[], values: unknown[]): unknown[] {
  const row = insertRow(values);
  rows.push(row);
  return [row];
}

function updateRoute(rows: FakeRouteRow[], values: unknown[]): unknown[] {
...
function routeRows(rows: FakeRouteRow[], values: unknown[]): unknown[] {
...
function conversationPage(rows: FakeRouteRow[], values: unknown[]): unknown[] {
  const userId = values[0];
  const matches = rows.filter((item) => item.user_id === userId);
  matches.sort((a, b) => b.updated_at.localeCompare(a.updated_at) ||
...
function conversationRow(item: FakeRouteRow) {
  return {
    session_id: item.id,
    title: item.title,
