...
/**
 * Neon Auth EdDSA verification lived twice — `workers/edge/identity/auth.ts` (edge `/v1`) and
...
describe("EdDSA verification stays a single shared primitive (issue #647)", () => {
  ...
  it("neither consumer hand-rolls its own EdDSA algorithm restriction (any quote style)", () => {
    // `workers/edge/identity/auth.ts` legitimately reads a JWT header's `alg` field to decide
