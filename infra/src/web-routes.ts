...
if (webRoutesEnabled) {
  ...
  // `/v1/conversations/...` and `/catalog/public/*` relative to its own
  // origin, so a staging hostname pointed wholly at the web Worker has no
  // chat at all and no anime page. `/catalog/public/*` is the one anonymous
  // catalog door (iter-5 S5.4); the surface it routes is declared in
