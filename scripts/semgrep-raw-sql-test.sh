...
#   (a) the ruleset FLAGS every representative forbidden example (complete
#       sql`` DML, sql.raw(), direct neon() client) — the scan must FAIL
#       (--error) on the forbidden fixtures.
#   (b) the ruleset PASSES the sanctioned exceptions (db/client.ts,
#       db/expressions.ts) — the scan must return ZERO findings.
#
# Why a temp-mirror scan: the rules carry `paths.include` scoped to
# /workers/..., and .semgrepignore ignores the .semgrep config
...
forbidden() {
  ...
  mkdir -p "${t}" "${root}/workers/users/src"
...
baseline() {
  ...
  # scan. The anchored paths.include (/workers/catalog/src, ...)
