"use client";

import { useState } from "react";
import type { PilgrimagePoint } from "../../lib/types";
import { handleImageError } from "../auth/LandingData";
import SpotCard from "./SpotCard";

const PREVIEW_COUNT = 6;

interface SpotGroupProps {
  title: string;
  count: number;
  points: PilgrimagePoint[];
  defaultOpen?: boolean;
  /** i18n label for "show all" — expects {count} placeholder */
  showAllLabel?: string;
  /** Ref callback for scroll-reveal on each card */
  revealRef?: (el: HTMLElement | null) => void;
  selectedIds?: Set<string>;
  onToggle?: (id: string) => void;
}

/** Pick the first screenshot_url from points, if any. */
function leadThumb(points: PilgrimagePoint[]): string | null {
  for (const p of points) {
    if (p.screenshot_url) return p.screenshot_url;
  }
  return null;
}

export default function SpotGroup({
  title,
  count,
  points,
  defaultOpen = false,
  showAllLabel,
  revealRef,
  selectedIds,
  onToggle,
}: SpotGroupProps) {
  const [open, setOpen] = useState(defaultOpen);
  const [showAll, setShowAll] = useState(false);
  const isSelectMode = selectedIds !== undefined && onToggle !== undefined;

  const hasMore = points.length > PREVIEW_COUNT;
  const visiblePoints = showAll || !hasMore ? points : points.slice(0, PREVIEW_COUNT);
  const thumb = !open ? leadThumb(points) : null;

  return (
    <div className="border-b border-[var(--color-border)]">
      <button
        type="button"
        onClick={() => setOpen((prev) => !prev)}
        className="flex w-full items-center justify-between rounded-lg px-2 py-4 text-left transition-colors hover:bg-[var(--color-card)]"
      >
        <h3
          className="flex items-center gap-2.5 text-[16px] font-semibold text-[var(--color-fg)]"
          style={{ fontFamily: "var(--app-font-display)" }}
        >
          {/* Leading thumbnail when collapsed */}
          {thumb && (
            <span className="inline-block h-7 w-7 shrink-0 overflow-hidden rounded-[6px]">
              <img
                src={thumb}
                alt=""
                className="h-full w-full object-cover"
                loading="lazy"
                onError={handleImageError}
              />
            </span>
          )}
          {title}
        </h3>
        <span className="flex items-center gap-2 text-[13px] text-[var(--color-muted-fg)]">
          {count} spots
          <span className="text-[10px]">{open ? "▼" : "▶"}</span>
        </span>
      </button>

      {open && (
        <div style={{ animation: "seichi-fade-up 0.3s cubic-bezier(0.16,1,0.3,1)" }}>
          <div className="grid grid-cols-1 gap-4 pb-4 sm:grid-cols-2 lg:grid-cols-3">
            {visiblePoints.map((point, i) => (
              <div
                key={point.id}
                ref={revealRef}
                className="seichi-reveal-pop"
                style={{ animationDelay: `${i * 0.04}s` }}
              >
                {isSelectMode ? (
                  <SpotCard
                    point={point}
                    mode="select"
                    selected={selectedIds.has(point.id)}
                    onToggle={onToggle}
                  />
                ) : (
                  <SpotCard point={point} mode="browse" />
                )}
              </div>
            ))}
          </div>
          {hasMore && !showAll && (
            <button
              type="button"
              onClick={() => setShowAll(true)}
              className="mb-4 w-full rounded-lg border border-[var(--color-border)] py-3 text-[14px] font-medium text-[var(--color-muted-fg)] transition-colors hover:bg-[var(--color-card)] hover:text-[var(--color-fg)]"
            >
              {showAllLabel
                ? showAllLabel.replace("{count}", String(count))
                : `Show all ${count} spots`}
            </button>
          )}
        </div>
      )}
    </div>
  );
}
