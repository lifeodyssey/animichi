...
def _feedback_id_from_payload(payload: JSONDict) -> str:
    feedback_id = payload.get("feedback_id")
    if not isinstance(feedback_id, str):
        raise ValueError("Expected feedback_id in response")
    return feedback_id
...
class SeichijunreiClient:
    ...
    def search(
        ...
        body: JSONDict = {"text": text, "locale": locale}
        if session_id:
    ...
    def feedback(
        ...
        body: JSONDict = {"query_text": query_text, "rating": rating}
        if intent:
            body["intent"] = intent
        if comment:
        ...
        response.raise_for_status()
        return _feedback_id_from_payload(_as_json_object(response.json()))
