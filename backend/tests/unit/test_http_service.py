...
from backend.interfaces.http_service import (
...
class TestHTTPService:
    ...
    async def test_runtime_endpoint_executes_public_api(self, mock_db):
        ...
        with patch(
            "backend.interfaces.public_api.run_pipeline", side_effect=fake_pipeline
        ):
            app = create_http_app(
                runtime_api=RuntimeAPI(mock_db, session_store=InMemorySessionStore()),
                settings=Settings(),
    ...
    async def test_runtime_endpoint_records_http_observability(self, mock_db):
        ...
        with (
            ...
            patch(
                "backend.interfaces.http_service.get_http_tracer",
                return_value=DummyTracer(span),
            ),
            patch(
                "backend.interfaces.http_service.record_http_request"
            ) as record_metric,
        ):
            request = make_mocked_request("POST", "/v1/runtime", app=app)
            request.json = AsyncMock(return_value={"text": "秒速5厘米的取景地在哪"})
    ...
    async def test_service_lifecycle_initializes_observability_when_enabled(self):
        ...
        with (
            patch("backend.interfaces.http_service.setup_observability") as setup_obs,
            patch(
                "backend.interfaces.http_service.shutdown_observability"
            ) as shutdown_obs,
        ):
            app = create_http_app(
                settings=settings,
