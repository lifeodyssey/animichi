...
_tracer_provider: _Shutdownable | None = None
...
def setup_tracing(
    ...
    **exporter_kwargs: object,
    ...
    try:
        ...
        from backend.infrastructure.observability.exporters import create_span_exporter
    ...
    if exporter_type != "none":
        ...
        if exporter is not None:
            provider.add_span_processor(
                BatchSpanProcessor(cast(SpanExporter, exporter))
            )
...
def get_tracer(name: str) -> _NoOpTracer:
    ...
    try:
        ...
        return cast(_NoOpTracer, trace.get_tracer(name))
    except ImportError:
