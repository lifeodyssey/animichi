...
function useDropdown(options: readonly SelectOption[], value: string): Dropdown {
  ...
  return { open, active, toggle, close: useCallback(() => { setOpen(false); }, []), setActive };
}
...
function SelectTrigger({ id, text, open, onToggle }: TriggerProps) {
  return (
    <button type="button" className="ds-select__trigger" role="combobox" aria-labelledby={`${id}-label`} aria-expanded={open} aria-haspopup="listbox" aria-controls={`${id}-listbox`} onClick={onToggle}>
      <span className="ds-select__value">{text}</span><span className="ds-select__caret" aria-hidden="true">▾</span>
    </button>
  );
}
...
export function Select({ id, label, value, options, onChange }: SelectProps) {
  const dropdown = useDropdown(options, value);
  const onPick = makePicker(options, value, dropdown, onChange);
  return (
    <div className="ds-select">
      <span className="ds-select__label" id={`${id}-label`}>{label}</span>
      <SelectTrigger id={id} text={labelOfValue(options, value)} open={dropdown.open} onToggle={dropdown.toggle} />
      <SelectList id={id} options={options} value={value} dropdown={dropdown} onPick={onPick} /></div>
  );
}
