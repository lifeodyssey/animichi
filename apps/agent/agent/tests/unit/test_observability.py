...
class TestObservabilityLifecycle:
    ...
    def test_setup_observability_configures_tracing_and_metrics(self):
        ...
        with (
            patch(
                "agent.infrastructure.observability.setup_tracing"
            ) as setup_tracing_mock,
            patch(
                "agent.infrastructure.observability.setup_metrics"
            ) as setup_metrics_mock,
            patch(
                "agent.infrastructure.observability.reset_runtime_observability"
            ) as reset_mock,
        ):
    ...
    def test_shutdown_observability_flushes_all_providers(self):
        with (
            patch(
                "agent.infrastructure.observability.shutdown_tracing"
            ) as shutdown_tracing_mock,
            patch(
                "agent.infrastructure.observability.shutdown_metrics"
