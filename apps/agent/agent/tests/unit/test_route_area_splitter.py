"""Unit tests for agent.agents.route_area_splitter."""

from __future__ import annotations

from unittest.mock import AsyncMock, MagicMock, patch

from agent.agents.route_area_splitter import (
    AreaGroup,
    AreaSplitResult,
    split_into_areas,
)
...
class TestSplitIntoAreasLargeSets:
    async def test_returns_result_for_fifteen_points(self) -> None:
        ...
        with patch(
            "agent.agents.route_area_splitter.route_planner_agent"
        ) as mock_agent:
...
class TestCalculateDistanceTool:
    def test_returns_correct_haversine_for_tokyo_osaka(self) -> None:
        from agent.agents.route_area_splitter import calculate_distance
