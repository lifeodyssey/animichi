...
from animichi.agents.route_area_splitter import split_into_areas
from animichi.config.model_aliases import (
    CredentialRef,
    ModelAlias,
...
def test_model_http_client_uses_attempt_timeout_and_environment() -> None:
    ...
    with patch(
        "animichi.agents.base.httpx.AsyncClient", return_value=expected
    ) as build:
...
def test_fastapi_lifespan_owns_and_closes_model_client() -> None:
    ...
    with patch(
        "animichi.interfaces.fastapi_service.build_model_http_client",
