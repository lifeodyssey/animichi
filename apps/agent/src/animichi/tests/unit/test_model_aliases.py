...
from animichi.agents.base import MODEL_ALIASES, ModelAliasError, resolve_model_alias
...
async def test_unknown_model_alias_never_runs_agent() -> None:
    with patch(
        "animichi.agents.animichi_runner.animichi_agent.run", new_callable=AsyncMock
    ) as run:
...
def test_default_alias_uses_existing_default_model() -> None:
    ...
    with patch("animichi.agents.base.get_default_model", return_value=expected):
...
def test_model_alias_registry_is_built_once() -> None:
    with patch("animichi.config.model_aliases._build_aliases") as build:
...
def test_mimo_alias_uses_server_owned_provider(mock_settings: Settings) -> None:
    with patch("animichi.config.get_settings", return_value=mock_settings):
...
def test_allowlisted_alias_may_contain_http_substring() -> None:
    ...
    alias = MagicMock()
    with (
        patch("animichi.agents.base.MODEL_ALIASES", {"http_model": alias}),
        patch("animichi.agents.base._parse_model_alias", return_value=expected),
    ):
...
async def test_none_model_override_is_forwarded_unchanged() -> None:
    ...
    with patch(
        "animichi.agents.animichi_runner.animichi_agent.run",
