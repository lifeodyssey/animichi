...
check directly on `animichi.config.settings.Settings`. Independent review (twice)
...
-> first import of `animichi.config.model_aliases`, whose module-level
...
a wholly separate class that never imports `animichi.config.model_aliases`, so
...
Uses `animichi.config.settings.get_settings` directly (not `animichi.config.get_settings`,
...
class TestPurgeCronSettings:
    ...
    def test_never_imports_model_aliases(self) -> None:
        ...
        `animichi.config.model_aliases` into `sys.modules` — that import is the
        ...
        literal `import animichi.config.model_aliases` in this one file.
        """
        env = dict(os.environ)
        ...
        result = subprocess.run(
            [
                ...
                "from animichi.config.cron_settings import get_purge_cron_settings\n"
                "get_purge_cron_settings()\n"
                "assert 'animichi.config.model_aliases' not in sys.modules, "
                "sorted(m for m in sys.modules if m.startswith('animichi.config'))\n",
            ],
            capture_output=True,
            text=True,
            env=env,
...
class TestGetSettingsSingletonIsUnaffected:
    ...
    process (importing `animichi.agents.translation`, which imports
    `animichi.config.model_aliases`, whose module-level alias build calls it).
