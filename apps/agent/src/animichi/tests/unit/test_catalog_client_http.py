...
from animichi.clients.catalog_errors import WorkNotFoundError
...
def _install_transport(
    ...
    monkeypatch.setattr(
        "animichi.clients.catalog_client.httpx.AsyncHTTPTransport", constructor
    )
    return constructor
...
def _no_sleep(monkeypatch: pytest.MonkeyPatch) -> AsyncMock:
    sleep = AsyncMock()
    monkeypatch.setattr("animichi.clients.catalog_client.asyncio.sleep", sleep)
    return sleep
