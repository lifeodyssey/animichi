...
``apps/agent/scripts/check_prompt_token_budget.py`` lives outside the
``src/animichi`` package (a standalone CI/dev script, like the root ``scripts/``
shell tools), so it is loaded here via its file path rather than a normal
package import.
"""
...
_SCRIPT_PATH = (
    Path(__file__).resolve().parents[4] / "scripts" / "check_prompt_token_budget.py"
)
_INSTRUCTIONS_PATH = (
