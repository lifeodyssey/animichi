 ...
 * and the `data-splash-release` rule in globals.css) — so the landing
 * underneath `/` is never uncovered in between, no matter how long the chat
 * chunk and loader take to arrive.
 *
 * The landing still renders server-side, so crawlers and share previews keep
 * getting it; only the human visitor is carried through.
 */
