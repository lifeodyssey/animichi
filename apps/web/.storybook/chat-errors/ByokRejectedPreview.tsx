...
type Props = Readonly<{ dict: ChatDict; withMessage?: boolean; sessionId: string | undefined }>;
const MESSAGE = {
...
export function byokRejectedPreviewDict(locale: unknown): ChatDict {
  return chatDictFor(typeof locale === "string" && isLocale(locale) ? locale : "zh");
}

/** Authored message and session fixtures; the component retains its real settings link. */
export function ByokRejectedPreview({ dict, withMessage, sessionId }: Props) {
  return <ChatReturnTargetProvider sessionIdOf={() => sessionId}>
    <div className="grid w-[min(452px,calc(100vw_-_80px))] max-w-full gap-6">
